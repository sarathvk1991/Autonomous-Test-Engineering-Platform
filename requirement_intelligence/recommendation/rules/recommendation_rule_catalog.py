"""The governed :class:`RecommendationRuleCatalog` — an ordered collection of
recommendation rules (CAP-082B).

A ``RecommendationRuleCatalog`` owns **ordering, lookup, grouping, and enabled-rule
selection** over a set of :class:`RecommendationRule` — and nothing else. It
performs **no recommendation generation**: it neither reads an upstream result nor a
policy value, and it makes no decision. The
:class:`~requirement_intelligence.recommendation.engine.DeterministicRecommendationEngine`
iterates the catalogue; the catalogue only holds and organises the governed rules
(mirroring the analogous rule catalogues in Quality Governance and Requirement
Enhancement).

Determinism
-----------
Rules are stored in a single canonical order — ascending ``evaluation_order``, then
``rule_id`` as a stable tie-break — so iteration, serialization, and every derived
grouping are reproducible run after run, independent of insertion order.
"""

from __future__ import annotations

from pydantic import ConfigDict, Field, model_validator
from pydantic.alias_generators import to_camel

from requirement_intelligence.recommendation.identity.recommendation_identity import (
    RecommendationRuleCatalogVersion,
)
from requirement_intelligence.recommendation.models.enums import RecommendationSource
from requirement_intelligence.recommendation.rules.recommendation_rule import (
    RecommendationRule,
    RecommendationRuleCategory,
)
from shared.contracts.base import Schema

#: Version of the governed default rule catalogue (CAP-082B foundation).
RECOMMENDATION_RULE_CATALOG_VERSION = RecommendationRuleCatalogVersion(1, 0, 0)


def _canonical_order(rule: RecommendationRule) -> tuple[int, str]:
    """The deterministic sort key: ``evaluation_order`` then ``rule_id``."""
    return (rule.evaluation_order, rule.rule_id)


class RecommendationRuleCatalog(Schema):
    """A deterministic, ordered collection of governed :class:`RecommendationRule`.

    The catalogue is immutable and self-contained. It exposes ordering, lookup by id
    or category, grouping by source subsystem, and the enabled-only projection the
    engine consumes. It owns no behaviour beyond organising its rules.
    """

    model_config = ConfigDict(alias_generator=to_camel)

    catalog_version: RecommendationRuleCatalogVersion = Field(
        default=RECOMMENDATION_RULE_CATALOG_VERSION,
        description="Version of this governed rule set.",
    )
    rules: tuple[RecommendationRule, ...] = Field(
        default=(), description="The governed rules, stored in canonical order."
    )

    @model_validator(mode="after")
    def _validate_catalog(self) -> RecommendationRuleCatalog:
        """Rules have unique ids and categories, and are stored in canonical order."""
        ids = [rule.rule_id for rule in self.rules]
        if len(ids) != len(set(ids)):
            raise ValueError("Catalog rule ids must be unique.")
        categories = [rule.category for rule in self.rules]
        if len(categories) != len(set(categories)):
            raise ValueError("Catalog rule categories must be unique (one rule per category).")
        if list(self.rules) != sorted(self.rules, key=_canonical_order):
            raise ValueError(
                "Catalog rules must be stored in canonical order "
                "(ascending evaluationOrder, then ruleId)."
            )
        return self

    def enabled_rules(self) -> tuple[RecommendationRule, ...]:
        """Return the enabled rules only, in canonical order.

        The engine iterates exactly this projection: a catalogue-disabled rule is not
        evaluated at all, distinct from a policy-toggled-off rule (which the engine
        skips at generation time by reading the rule's ``policy_reference``).
        """
        return tuple(rule for rule in self.rules if rule.enabled)

    def rule(self, rule_id: str) -> RecommendationRule | None:
        """Return the rule with *rule_id*, or ``None`` when absent."""
        for candidate in self.rules:
            if candidate.rule_id == rule_id:
                return candidate
        return None

    def rule_for_category(self, category: RecommendationRuleCategory) -> RecommendationRule | None:
        """Return the enabled rule naming *category*, or ``None`` when absent/disabled."""
        for candidate in self.enabled_rules():
            if candidate.category == category:
                return candidate
        return None

    def by_source(self, source: RecommendationSource) -> tuple[RecommendationRule, ...]:
        """Return the rules concerning *source*, in canonical order."""
        return tuple(rule for rule in self.rules if rule.source_subsystem == source)
