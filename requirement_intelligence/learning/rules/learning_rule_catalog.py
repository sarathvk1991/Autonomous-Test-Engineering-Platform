"""The governed :class:`LearningRuleCatalog` — an ordered collection of
Learning rules (CAP-086B).

A ``LearningRuleCatalog`` owns **ordering, lookup, grouping, and
enabled-rule selection** over a set of :class:`LearningRule` — and nothing
else. It performs **no collection, no clustering, no generation, no
decision**: it consumes no ``OrganizationalMemoryResult``. The deterministic
engine's collaborators iterate the catalogue; the catalogue only holds and
organises the governed rules (mirroring the Organizational Memory
Framework's own governed rule catalogue, ADR-0027).
...
#: Version of the governed default rule catalogue (CAP-086B foundation).
LEARNING_RULE_CATALOG_VERSION = LearningRuleCatalogVersion(1, 0, 0)


def _canonical_order(rule: LearningRule) -> tuple[int, str]:
    """The deterministic sort key: ``evaluation_order`` then ``rule_id``."""
    return (rule.evaluation_order, rule.rule_id)


class LearningRuleCatalog(Schema):
    """A deterministic, ordered collection of governed :class:`LearningRule`.
    ...
    catalog_version: LearningRuleCatalogVersion = Field(
        default=LEARNING_RULE_CATALOG_VERSION,
        description="Version of this governed rule set.",
    )
    rules: tuple[LearningRule, ...] = Field(
        default=(), description="The governed rules, stored in canonical order."
    )

    @model_validator(mode="after")
    def _validate_catalog(self) -> LearningRuleCatalog:
    ...
    def enabled_rules(self) -> tuple[LearningRule, ...]:
    ...
    def rule(self, rule_id: str) -> LearningRule | None:
    ...
    def by_category(self, category: LearningRuleCategory) -> tuple[LearningRule, ...]:
    ...
    def by_hierarchy_level(self, level: LearningHierarchyLevel) -> tuple[LearningRule, ...]:
