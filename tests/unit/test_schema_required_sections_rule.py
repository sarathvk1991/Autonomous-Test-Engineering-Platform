"""Unit tests for SCHEMA-0001 RequiredSectionsRule.

Covers metadata/identity, happy path, missing-section findings, the per-occurrence
(one-issue-per-missing-section) behaviour, read-only ownership (spy-proven), input
immutability, determinism, facts-vs-exceptions, deferral on an absent structure, and
registration / pipeline / validator integration.
...
* Per ADR-0003 the rule receives a ``ValidationInput``; per ADR-0004 it owns
  existence of required **non-collection** sections (the governed set is ``summary``).
...
# The governed schema has one non-collection required section (ADR-0004).
_WITH_SUMMARY = '{"summary": "s", "risks": []}'
_WITHOUT_SUMMARY = '{"risks": [], "recommendations": []}'
...
class _SpyParsedResponse:
    ...
    def normalization_outcome(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("SCHEMA-0001 must not inspect the normalization outcome")

    @property
    def source_reference(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("SCHEMA-0001 must not inspect source_reference")

    @property
    def metadata(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("SCHEMA-0001 must not inspect ParsedResponse metadata")
...
class _SpyNormalizationResult:
    ...
    def observations(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("SCHEMA-0001 must not inspect observations")
...
class _SpyAnalysisResult:
    ...
    def llm_response(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("SCHEMA-0001 must not inspect llm_response/generated_text/provider")
...
class TestMetadata:
    def test_is_a_validation_rule(self) -> None:
        assert isinstance(RequiredSectionsRule(), ValidationRule)

    def test_rule_id(self) -> None:
        assert RequiredSectionsRule().rule_id == "SCHEMA-0001"

    def test_rule_name(self) -> None:
        assert RequiredSectionsRule().rule_name == "Required Sections"

    def test_layer(self) -> None:
        assert RequiredSectionsRule().validation_layer == ValidationLayer.SCHEMA

    def test_rule_version_is_default(self) -> None:
        assert RequiredSectionsRule().rule_version == DEFAULT_RULE_VERSION

    def test_enabled_by_default(self) -> None:
        assert RequiredSectionsRule().enabled is True
    ...
    def test_severity_and_blocking_from_catalog(self) -> None:
        # Catalog §14: a missing required section is ERROR (→ FAILED), not
        # CRITICAL/BLOCKED, so the finding is non-blocking (§15).
        issue = RequiredSectionsRule().validate(_input(_WITHOUT_SUMMARY))[0]
...
@pytest.mark.unit
class TestHappyPath:
    def test_required_section_present_yields_no_issues(self) -> None:
        assert RequiredSectionsRule().validate(_input(_WITH_SUMMARY)) == []

    def test_success_returns_empty_list_type(self) -> None:
        result = RequiredSectionsRule().validate(_input(_WITH_SUMMARY))
        assert isinstance(result, list)
        assert len(result) == 0

    def test_present_via_spy_yields_no_issues(self) -> None:
        assert RequiredSectionsRule().validate(_spy_input({"summary": "s"})) == []


# ---------------------------------------------------------------------------
# 3. Missing section & issue contents
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestMissingSection:
    def test_missing_section_yields_one_issue(self) -> None:
        assert len(RequiredSectionsRule().validate(_input(_WITHOUT_SUMMARY))) == 1

    def test_issue_identity_fields(self) -> None:
        issue = RequiredSectionsRule().validate(_input(_WITHOUT_SUMMARY))[0]
        assert issue.rule_id == "SCHEMA-0001"
        assert issue.rule_version == DEFAULT_RULE_VERSION
        assert issue.issue_id == "SCHEMA-0001:summary"
    ...
    def test_issue_message_and_recommendation(self) -> None:
        issue = RequiredSectionsRule().validate(_input(_WITHOUT_SUMMARY))[0]
        assert issue.message == "The required section 'summary' is missing from the response."
        assert issue.recommendation == "Include the required 'summary' section in the response."
    ...
    def test_issue_correlation_from_execution_id(self) -> None:
        issue = RequiredSectionsRule().validate(_input(_WITHOUT_SUMMARY, execution_id="EX-9"))[0]
    ...
    def test_missing_via_spy_yields_one_issue(self) -> None:
        assert len(RequiredSectionsRule().validate(_spy_input({"risks": []}))) == 1


# ---------------------------------------------------------------------------
# 4. Per-occurrence behaviour (one issue per missing section)
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestPerOccurrence:
    """One ``ValidationIssue`` per missing required section (Implementation Contract
    §5 — per occurrence, not aggregated).  The governed set has a single
    non-collection section, so the general behaviour is verified by temporarily
    widening the governed constant (production behaviour is unchanged)."""

    def test_one_issue_per_missing_section(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            required_sections_rule,
            "_REQUIRED_NON_COLLECTION_SECTIONS",
            ("summary", "overview"),
        )
        issues = RequiredSectionsRule().validate(_input('{"risks": []}'))
        assert len(issues) == 2
        assert {i.location for i in issues} == {"summary", "overview"}

    def test_only_missing_sections_reported(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            required_sections_rule,
            "_REQUIRED_NON_COLLECTION_SECTIONS",
            ("summary", "overview"),
        )
        issues = RequiredSectionsRule().validate(_input('{"summary": "s"}'))
        assert [i.location for i in issues] == ["overview"]
...
class TestReadsOnlyStructure:
    ...
    def test_does_not_inspect_forbidden_fields_on_finding(self) -> None:
        assert len(RequiredSectionsRule().validate(_spy_input({}))) == 1
...
class TestRuleIndependence:
    ...
    def test_does_not_mutate_validation_input(self) -> None:
        rule = RequiredSectionsRule()
        validation_input = _input(_WITHOUT_SUMMARY)
    ...
    def test_idempotent_no_cumulative_effect(self) -> None:
        rule = RequiredSectionsRule()
        missing = _input(_WITHOUT_SUMMARY)
        assert len(rule.validate(missing)) == 1
        assert len(rule.validate(missing)) == 1
        present = _input(_WITH_SUMMARY)
...
class TestFactsNotExceptions:
    def test_missing_section_is_a_finding_not_an_exception(self) -> None:
        issues = RequiredSectionsRule().validate(_input(_WITHOUT_SUMMARY))
        assert len(issues) == 1
    ...
    def test_defers_on_absent_structure(self) -> None:
        ...
        assert RequiredSectionsRule().validate(_input(_MALFORMED)) == []

    def test_missing_parsed_response_raises_infrastructure_error(self) -> None:
        analysis = _analysis_result()
        broken = _normalization_result(analysis, _WITH_SUMMARY).model_copy(
        ...
        with pytest.raises(ValidationRuleError):
            RequiredSectionsRule().validate(validation_input)
...
class TestRegistration:
    def test_register_schema_rules_includes_this_rule(self) -> None:
        ...
        assert "SCHEMA-0001" in registry.list_rule_ids()
    ...
    def test_rule_can_be_registered_directly(self) -> None:
        ...
        assert len(rules) == 1
        assert isinstance(rules[0], RequiredSectionsRule)
...
class TestPipelineAndValidator:
    ...
    def test_validator_fails_on_missing_section(self) -> None:
        registry = ValidationRegistry()
        registry.register(RequiredSectionsRule())
        validator = ResponseValidator(
            registry, ValidationPipeline(registry), ValidationConfiguration()
        )
        result = validator.validate(_input(_WITHOUT_SUMMARY))
        assert result.overall_verdict == ValidationVerdict.FAILED
        assert [i.rule_id for i in result.validation_issues] == ["SCHEMA-0001"]
