...
class TestStageDefinitions:
    ...
    def test_live_and_placeholder_counts(self) -> None:
        assert len(LIVE_STAGE_IDS) == 14  # 13 ADR-0036 stages + TRS emission
        assert len(PLACEHOLDER_STAGE_IDS) == 6  # Layers 2-7
    ...
    def test_placeholder_layers_cover_l2_through_l7(self) -> None:
        placeholder_layers = {
            d.layer for d in STAGE_DEFINITIONS if d.stage_id in PLACEHOLDER_STAGE_IDS
        }
        assert placeholder_layers == {"L2", "L3", "L4", "L5", "L6", "L7"}
