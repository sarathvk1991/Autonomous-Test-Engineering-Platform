"""Unit tests for REASONING-0002 DuplicateRecommendationRule.

Covers metadata/identity, happy path, duplicate findings and per-occurrence counting,
byte-exact / case-sensitive / whitespace-sensitive matching (ADR-0008), the ownership
boundary (only ``recommendations`` inspected; requirements/summary/risks ignored;
...
* Per ADR-0003 the rule receives a ``ValidationInput``; per Rule Catalog §9.8 and
  ADR-0008 it owns **byte-exact** duplication of the single ``recommendations`` collection.
...
from requirement_intelligence.validation.rules.reasoning import (
...
_NO_DUP = json.dumps(
    {
        "summary": "s",
        "functional_requirements": ["A", "A"],  # a requirement duplicate → ignored here
        "risks": ["R", "R"],  # not a recommendation → ignored
        "recommendations": ["A", "B", "C"],
    }
)
_ONE_DUP = json.dumps({"recommendations": ["A", "B", "A"]})
...
class _SpyParsedResponse:
    ...
    def normalization_outcome(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("REASONING-0002 must not inspect the normalization outcome")

    @property
    def source_reference(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("REASONING-0002 must not inspect source_reference")

    @property
    def metadata(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("REASONING-0002 must not inspect ParsedResponse metadata")
...
class _SpyNormalizationResult:
    ...
    def observations(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("REASONING-0002 must not inspect observations")
...
class _SpyAnalysisResult:
    ...
    def llm_response(self) -> Any:  # pragma: no cover - must never run
        raise AssertionError("REASONING-0002 must not inspect llm_response/generated_text/provider")
...
class TestMetadata:
    def test_is_a_validation_rule(self) -> None:
        assert isinstance(DuplicateRecommendationRule(), ValidationRule)

    def test_rule_id(self) -> None:
        assert DuplicateRecommendationRule().rule_id == "REASONING-0002"

    def test_rule_name(self) -> None:
        assert DuplicateRecommendationRule().rule_name == "Duplicate Recommendation"

    def test_layer(self) -> None:
        assert DuplicateRecommendationRule().validation_layer == ValidationLayer.REASONING

    def test_rule_version_is_default(self) -> None:
        assert DuplicateRecommendationRule().rule_version == DEFAULT_RULE_VERSION

    def test_enabled_by_default(self) -> None:
        assert DuplicateRecommendationRule().enabled is True

    def test_metadata_is_stable_across_access(self) -> None:
        rule = DuplicateRecommendationRule()
        assert rule.metadata is rule.metadata

    def test_severity_and_blocking_from_catalog(self) -> None:
        # Catalog §14: a duplicate recommendation is ERROR (→ FAILED); §15: Reasoning is a
        # semantic layer that records and never blocks.
        issue = DuplicateRecommendationRule().validate(_input(_ONE_DUP))[0]
...
class TestHappyPath:
    def test_no_duplicates_yields_no_issues(self) -> None:
        assert DuplicateRecommendationRule().validate(_input(_NO_DUP)) == []

    def test_success_returns_empty_list_type(self) -> None:
        result = DuplicateRecommendationRule().validate(_input(_NO_DUP))
        assert isinstance(result, list)
        assert len(result) == 0

    def test_no_recommendations_present_yields_no_issues(self) -> None:
        assert DuplicateRecommendationRule().validate(_spy_input({"summary": "s"})) == []
...
class TestDuplicate:
    def test_single_duplicate_yields_one_issue(self) -> None:
        assert len(DuplicateRecommendationRule().validate(_input(_ONE_DUP))) == 1

    def test_issue_identity_fields(self) -> None:
        # ["A","B","A"] → the second "A" at index 2 is the duplicate occurrence.
        issue = DuplicateRecommendationRule().validate(_input(_ONE_DUP))[0]
        assert issue.rule_id == "REASONING-0002"
        assert issue.rule_version == DEFAULT_RULE_VERSION
        assert issue.issue_id == "REASONING-0002:recommendations[2]"
        assert issue.location == "recommendations[2]"

    def test_issue_layer_and_category(self) -> None:
        issue = DuplicateRecommendationRule().validate(_input(_ONE_DUP))[0]
        assert issue.validation_layer == ValidationLayer.REASONING.value
        assert issue.category == "reasoning"

    def test_issue_message_and_recommendation(self) -> None:
        issue = DuplicateRecommendationRule().validate(_input(_ONE_DUP))[0]
        assert issue.message == (
            "The recommendation at recommendations[2] duplicates an earlier "
            "recommendation in 'recommendations'."
        )
        assert issue.recommendation == (
            "Remove the duplicate recommendation at recommendations[2] "
            "or replace it with a distinct recommendation."
        )
    ...
    def test_issue_correlation_from_execution_id(self) -> None:
        issue = DuplicateRecommendationRule().validate(_input(_ONE_DUP, execution_id="EX-9"))[0]
...
class TestPerOccurrence:
    def test_two_extra_occurrences_yield_two_issues(self) -> None:
        ...
        structure = {"recommendations": ["A", "B", "A", "A"]}
        issues = DuplicateRecommendationRule().validate(_spy_input(structure))
    ...
    def test_first_occurrence_never_reported(self) -> None:
        structure = {"recommendations": ["A", "A"]}
        issues = DuplicateRecommendationRule().validate(_spy_input(structure))
        assert [i.location for i in issues] == ["recommendations[1]"]

    def test_multiple_distinct_duplicates(self) -> None:
        structure = {"recommendations": ["A", "A", "B", "B"]}
        issues = DuplicateRecommendationRule().validate(_spy_input(structure))
        assert [i.location for i in issues] == ["recommendations[1]", "recommendations[3]"]


# ---------------------------------------------------------------------------
# 5. Byte-exact, case-sensitive, whitespace-sensitive matching (ADR-0008)
# ---------------------------------------------------------------------------
...
class TestExactMatching:
    def test_case_sensitive_not_a_duplicate(self) -> None:
        assert (
            DuplicateRecommendationRule().validate(_spy_input({"recommendations": ["A", "a"]}))
            == []
        )
    ...
    def test_byte_exact_duplicate_is_reported(self) -> None:
        issues = DuplicateRecommendationRule().validate(
            _spy_input({"recommendations": ["A A", "A A"]})
        )
        assert len(issues) == 1


# ---------------------------------------------------------------------------
# 6. Ownership boundary — recommendations only; other sections & non-list/non-string
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestOwnershipBoundary:
    def test_requirements_ignored(self) -> None:
        # Duplicate requirement statements are CONTENT-0002's concern, not this rule's.
        structure = {"functional_requirements": ["A", "A"], "recommendations": ["X"]}
        assert DuplicateRecommendationRule().validate(_spy_input(structure)) == []

    def test_summary_ignored(self) -> None:
        assert DuplicateRecommendationRule().validate(_spy_input({"summary": "s"})) == []

    def test_risks_ignored(self) -> None:
        assert DuplicateRecommendationRule().validate(_spy_input({"risks": ["R", "R"]})) == []

    def test_absent_recommendations_skipped(self) -> None:
        assert DuplicateRecommendationRule().validate(_spy_input({})) == []

    def test_empty_recommendations_skipped(self) -> None:
        assert DuplicateRecommendationRule().validate(_spy_input({"recommendations": []})) == []

    def test_non_list_recommendations_skipped(self) -> None:
        # A present-but-non-list value is SCHEMA-0002's concern (type), not this rule's.
        assert DuplicateRecommendationRule().validate(_spy_input({"recommendations": "AA"})) == []

    def test_non_string_elements_skipped(self) -> None:
        structure = {"recommendations": [1, 1, None, None, {"a": 1}, {"a": 1}]}
        assert DuplicateRecommendationRule().validate(_spy_input(structure)) == []

    def test_non_string_duplicate_ignored_but_string_duplicate_found(self) -> None:
        structure = {"recommendations": [1, 1, "A", "A"]}
        issues = DuplicateRecommendationRule().validate(_spy_input(structure))
        assert [i.location for i in issues] == ["recommendations[3]"]


# ---------------------------------------------------------------------------
# 7. Reads only the normalized structure
# ---------------------------------------------------------------------------
...
class TestReadsOnlyStructure:
    def test_does_not_inspect_forbidden_fields_on_pass(self) -> None:
        assert DuplicateRecommendationRule().validate(_spy_input({"recommendations": ["A"]})) == []

    def test_does_not_inspect_forbidden_fields_on_finding(self) -> None:
        assert (
            len(DuplicateRecommendationRule().validate(_spy_input({"recommendations": ["A", "A"]})))
            == 1
        )


# ---------------------------------------------------------------------------
# 8. Immutability, determinism, idempotency
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 9. Facts vs exceptions & deferral
# ---------------------------------------------------------------------------
...
class TestFactsNotExceptions:
    def test_duplicate_is_a_finding_not_an_exception(self) -> None:
        issues = DuplicateRecommendationRule().validate(_input(_ONE_DUP))
        assert len(issues) == 1
    ...
    def test_defers_on_absent_structure(self) -> None:
        ...
        assert DuplicateRecommendationRule().validate(_input(_MALFORMED)) == []
    ...
    def test_missing_parsed_response_raises_infrastructure_error(self) -> None:
        ...
        with pytest.raises(ValidationRuleError):
            DuplicateRecommendationRule().validate(validation_input)


# ---------------------------------------------------------------------------
# 10. Registration, pipeline, validator integration
# ---------------------------------------------------------------------------


@pytest.mark.unit
class TestRegistration:
    def test_register_reasoning_rules_includes_this_rule(self) -> None:
        registry = ValidationRegistry()
        register_reasoning_rules(registry)
        assert "REASONING-0002" in registry.list_rule_ids()
    ...
    def test_rule_can_be_registered_directly(self) -> None:
        ...
        assert len(rules) == 1
        assert isinstance(rules[0], DuplicateRecommendationRule)
...
class TestPipelineAndValidator:
    ...
    def test_validator_fails_on_duplicate(self) -> None:
        ...
        assert result.overall_verdict == ValidationVerdict.FAILED
        assert [i.rule_id for i in result.validation_issues] == ["REASONING-0002"]
