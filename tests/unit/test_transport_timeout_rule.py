...
class TestRegistration:
    def test_transport_rules_registered_in_order(self) -> None:
        # ``register_transport_rules`` registers the whole Transport set; assert
        # membership and the relative order of 0001 → 0002 → 0003 (not the exact
        # set, which the newest rule's suite owns).
...
# ---------------------------------------------------------------------------
# 6. Pipeline & Response Validator integration (0001 + 0002 + 0003 in isolation)
# ---------------------------------------------------------------------------


def _validator() -> ResponseValidator:
    # Isolate the 0001 + 0002 + 0003 interaction so these counts stay stable as
    # the Transport rule set grows; the full set is covered by the newest suite.
    registry = ValidationRegistry()
