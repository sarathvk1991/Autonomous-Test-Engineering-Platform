"""ADR-0050's artifact-generation cache, extended to page objects:
`CachingPageObjectGenerator` wrapping `LivePageObjectGenerator` -- the
fourth generator covered (after step-def, feature-content, test-data).

No test in this module calls a real LLM -- the same hand-written
`FakeProvider` pattern
`test_automation_engineering_generation_page_object_generator.py` uses, so
the wrapped `LivePageObjectGenerator` is real, only the provider underneath
it is a fake. Mirrors
`test_automation_engineering_generation_caching_step_definition_generator.py`'s
own test structure exactly, plus one page-object-specific section this
generator has that step-defs don't: the BIND/CACHE COMPOSITION proof (a
page-object need the reuse engine BINDS to an existing tracked asset never
reaches this decorator's own `generate` at all -- bind and cache are two
different, non-conflicting reuse mechanisms).
"""
...
from automation_engineering.catalog.models import AssetCatalog, JavaMethod, PageObjectAsset
from automation_engineering.generation.caching_page_object_generator import (
    CachingPageObjectGenerator,
...
from automation_engineering.generation.models import BoundPageObjectMethod, PageObjectMethodNeed
...
def _context(
    ...
    defaults: dict[str, object] = {
        ...
        "method_name": _DEFAULT_METHOD_NAME,
    }
...
def _any_method_java() -> str:
    """Declares every method name this test module's own fixtures ever
    request -- a provider response that satisfies `LivePageObjectGenerator`'s
    own completeness check (module docstring: "missing (or renamed) one or
    more of the requested methods") no matter which subset a given test's
    context actually names, so a MISS test can vary `method_name`/
    `additional_method_needs` freely without also having to vary the
    canned LLM response to match."""
    return (
        "package com.automation.pages;\n\n"
        "public class ForgotPasswordLinkPage extends BasePage {\n"
...
class FakeProvider(LLMProvider):
    """Mirrors `test_automation_engineering_generation_page_object_
    generator.py`'s own `FakeProvider` exactly."""
    ...
    def __init__(
        ...
        *,
        ...
        self._text = text if text is not None else _single_method_java()
...
) -> tuple[CachingPageObjectGenerator, FakeProvider]:
    provider = inner_provider if inner_provider is not None else FakeProvider()
    inner = LivePageObjectGenerator(provider, usage_recorder=usage_recorder)
    identity = resolve_page_object_identity(
        **(identity_kwargs or _MATCHING_IDENTITY_KWARGS)  # type: ignore[arg-type]
    )
    decorator = CachingPageObjectGenerator(inner, store, identity, usage_recorder=usage_recorder)
...
class TestHitCorrectness:
    ...
    def test_hit_returns_the_identical_text_a_fresh_generation_would_have(
        ...
        independent_result = LivePageObjectGenerator(independent_provider).generate(context)
    ...
    def test_two_independent_decorator_instances_sharing_a_store_hit_across_instances(
        ...
        second_provider = FakeProvider(text="package com.automation.pages;\n// never reached\n")
        second_decorator, _ = _decorator(store=store, inner_provider=second_provider)
        result = second_decorator.generate(context)

        assert result == _single_method_java()  # the FIRST decorator's own output
        assert second_provider.call_count == 0  # the second instance's LLM was never called


# ---------------------------------------------------------------------------
# MISS on a changed input -- class + method needs + signatures, the real
# page-object generation payload
# ---------------------------------------------------------------------------


class TestMissOnChange:
    def test_a_changed_method_name_misses(self, tmp_path: Path) -> None:
        store = GenerationCacheStore(tmp_path)
        decorator, provider = _decorator(
            store=store, inner_provider=FakeProvider(text=_any_method_java())
        )

        decorator.generate(_context(method_name="clickForgotPasswordLink"))
        decorator.generate(_context(method_name="clickSignUpLink"))

        assert provider.call_count == 2  # both reached the LLM -- no stale hit

    def test_a_changed_class_name_with_method_name_unchanged_misses(self, tmp_path: Path) -> None:
        """The exact shape of the naive-key defect ADR-0050 D1 found: the
        method name (the closest L3 analogue of a `REQ-*` id) stays
        IDENTICAL while `class_name` changes. A correct cache must MISS."""
        store = GenerationCacheStore(tmp_path)
        decorator, provider = _decorator(store=store)

        decorator.generate(_context(class_name="ForgotPasswordLinkPage"))
        decorator.generate(_context(class_name="LoginPage"))
    ...
    def test_a_changed_return_type_signature_misses(self, tmp_path: Path) -> None:
        """A signature change (return type) with class+method name
        unchanged -- exactly the "signatures" half of the page-object
        payload (class + method needs + signatures)."""
        store = GenerationCacheStore(tmp_path)
        decorator, provider = _decorator(store=store)

        decorator.generate(_context(return_type=None))
        decorator.generate(_context(return_type="boolean"))

        assert provider.call_count == 2

    def test_an_additional_method_need_misses(self, tmp_path: Path) -> None:
        """The multi-method batching path (`additional_method_needs`) is
        part of the payload too -- a class generated for ONE method vs. TWO
        methods is a materially different generation, must MISS."""
        store = GenerationCacheStore(tmp_path)
        decorator, provider = _decorator(
            store=store, inner_provider=FakeProvider(text=_any_method_java())
        )
        extra = PageObjectMethodNeed(need=_need("enter the password"), method_name="enterPassword")

        decorator.generate(_context())
        decorator.generate(_context(additional_method_needs=(extra,)))
    ...
    def test_two_calls_with_identical_payloads_hit_regardless_of_object_identity(
        ...
        constructed contexts with the SAME class + method needs + signatures
...
# ---------------------------------------------------------------------------
# Additive: a MISS behaves exactly as an unwrapped LivePageObjectGenerator
# ---------------------------------------------------------------------------
...
class TestAdditiveOnMiss:
    def test_miss_sends_the_identical_prompt_a_bare_live_generator_would(
        ...
        LivePageObjectGenerator(bare_provider).generate(context)
...
# deterministic shape the step-def/feature-content/test-data increments
# measured live (real token counts falling to 0 on re-run) -- here proven
# deterministically (stub-shaped, no live LLM in `make test`); the live
# number is the same shape, available once page-object generation runs live.
# ---------------------------------------------------------------------------
...
class TestTwoPassMeasure:
    def test_pass_two_with_identical_inputs_makes_zero_generation_calls(
        ...
        store = GenerationCacheStore(tmp_path)
        method_names = ("clickForgotPasswordLink", "clickSignUpLink", "enterPassword")
        contexts = [
            _context(_need(f"do the {name} action"), class_name=f"{name}Page", method_name=name)
            for name in method_names
        ]

        # Pass 1 -- three DIFFERENT fresh generations, one real LLM call each.
        pass_one_provider = FakeProvider(text=_any_method_java())
...
# ---------------------------------------------------------------------------
# THE BIND/CACHE COMPOSITION -- page-object generation's own extra reuse
# mechanism the step-def/feature-content/test-data caches never had to
# compose with. Bind (an existing tracked PageObjectAsset already satisfies
# this need) and cache (we already generated this exact thing before) are
# orthogonal: bind never reaches this decorator's own generate() at all.
# ---------------------------------------------------------------------------
...
class TestBindNeverReachesTheCachingGenerator:
    def test_a_trusted_reuse_bind_never_calls_generate_or_touches_the_cache(
        ...
        asset = PageObjectAsset(
            asset_id="PAGE-login1",
            class_name="com.automation.pages.LoginPage",
            extends="BasePage",
            fields=(),
            locators=(),
            methods=(JavaMethod(name=_DEFAULT_METHOD_NAME, parameters=(), return_type="void"),),
            source_file="com/automation/pages/LoginPage.java",
            content_hash="current-hash",
        )
        catalog = AssetCatalog(baseline_root="test-suite-baseline", page_objects=(asset,))
        method_need = PageObjectMethodNeed(need=_need(), method_name=_DEFAULT_METHOD_NAME)
        ...
        outcome = orchestrate_page_object_method(method_need, catalog, matcher, decorator)
    ...
    def test_a_no_match_need_still_reaches_the_cache_normally(self, tmp_path: Path) -> None:
        ...
        first = orchestrate_page_object_method(method_need, catalog, matcher, decorator)
        second = orchestrate_page_object_method(method_need, catalog, matcher, decorator)
