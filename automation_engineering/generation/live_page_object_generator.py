...
``generate_page_objects`` prompt (v1.0.0) with one
...
``generate_page_objects`` v1.0.0 conforms to the full governed system/user
template contract (exactly one ``{artifact_context}`` placeholder), the same
as ``generate_step_definitions`` v1.0.0 -- rendered via
``render_user_prompt``, not an append-a-final-section workaround.
"""
...
class LivePageObjectGenerator:
    """``generate_page_objects`` v1.0.0-backed page-object generator.
    ...
    prompt_registry:
        ...
        ``generate_page_objects`` v1.0.0 from. When *None*, the canonical
    ...
    def _build_prompt(self, context: PageObjectGenerationContext) -> str:
        ...
        ``generate_page_objects`` v1.0.0's own INPUT CONTRACT names.
        """
        input_payload = {
