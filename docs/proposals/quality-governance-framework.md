...
- **Writer & manifest** — the writer conditionally appends `quality_governance_result.json`, `quality_governance_report.md`, `quality_governance_summary.md`; they enter `manifest.generatedArtifacts` via the existing checksum mechanism (schema unchanged). Additive CP1-pattern keys (`qualityGovernanceExecuted`, `qualityGovernanceReport`, `qualityGovernanceSummary`) reference the three artifacts by name. *(Superseded by §8f: the original cut also added `qualityGovernanceDecision`, surfacing the verdict directly on the manifest — CAP-080D.1 removed it.)*
...
## 8f. Manifest purity & runtime boundary hardening (CAP-080D.1)

CAP-080D.1 is a pure hardening pass over CAP-080D's integration — no engine, model, policy, or signature changes anywhere in the subsystem (ADR-0017 §D31). It removes the one manifest field, `qualityGovernanceDecision`, that duplicated runtime state (the release verdict) onto `manifest.json`, and freezes the boundary so it cannot regrow.

- **Manifest ownership (frozen).** `manifest.json` owns package identity, package metadata, execution metadata, the artifact inventory, checksums, and version information — nothing else. For Quality Governance this is exactly three keys: `qualityGovernanceExecuted` (execution metadata) and `qualityGovernanceReport` / `qualityGovernanceSummary` (artifact filenames). It never carries a decision, a score, findings, or recommendations — those answer *what was decided*, not *what package is this*.
- **Runtime ownership (frozen).** `QualityGovernanceResult` remains the sole owner of every governance runtime concept. The audit in ADR-0017 §D31 confirms each of `QualityDecision`, the overall quality score, governance findings, the assessment, and release readiness resolves to exactly one canonical field. Recommendations are documented as a gap (never propagated past the Decision layer into `QualityGovernanceResult`) rather than a duplication — closing that gap is future work, and it belongs to the runtime contract, never to the manifest.
- **Execution Package responsibility (frozen).** `ExecutionWriter`, `ManifestBuilder`, and `QualityGovernanceSerializer` together own only serialization, formatting, packaging, indexing, and checksums — never evaluation, assessment, classification, decision, or computation. Containment tests (`tests/unit/test_manifest_purity_boundary.py`) enforce that `ManifestBuilder`/`ExecutionData`/`ExecutionWriter` import no governance evaluator, assessment engine, decision engine, pipeline, builder, policy, or service — only the projection-only `serialization` submodule.
- **One-way dependency (frozen, permanent).** `manifest.json` never flows back into the runtime and never computes a decision; the Execution Package never reaches past the frozen `QualityGovernanceResult` contract into a runtime implementation.
- **Future consumers.** CI/CD, dashboards, release tooling, governance reports, and compliance tooling integrate against `quality_governance_result.json` — never against an expanded `manifest.json`. The manifest stays intentionally minimal; growing it is the drift this section forbids.
- **Golden regression.** The determinism test that diffed `manifest.qualityGovernanceDecision` across two runs was replaced with the equivalent runtime-contract check (already present) plus a metadata-only manifest-key check — a future Markdown or manifest-formatting change can never fail a runtime regression; only a `QualityGovernanceResult` content change can.
