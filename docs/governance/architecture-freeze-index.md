...
| **Validation Rule Catalog** | `RULE_CATALOG_VERSION` 1.0.0 | Approved — foundational | **Frozen** (catalog governance; grows/evolves via ADR) | Not Recorded | `docs/architecture/validation-rule-catalog.md` · ADR-0004 · ADR-0005 | AI Response Validation · Validation Canonical Models | Transport + Syntax rules implemented | The `<LAYER>-NNNN` rule numbering + layer order. **Schema ↔ Structural ownership set by ADR-0004** (existence → Schema; composition → Structural; `STRUCTURE-0001…0004` Deprecated). **`SCHEMA-0003` deferred by ADR-0005** (Reserved · Awaiting governed enumeration; no ID/version change). |
