...
| **Validation Rule Implementation Contract** | Contract 1.0 | Approved — foundational — **FROZEN** | **Frozen** (Architecture Freeze §14) | Not Recorded | `docs/architecture/validation-rule-implementation-contract.md` | Validation Rule Catalog · Validation Canonical Models (`ValidationIssue`/`ValidationInput`) · ValidationRule (Framework) · ADR-0003 | Transport rules (`TRANSPORT-0001…0004`) conform + tested | Governs the engineering structure every validation rule (all layers) follows (lifecycle, single concern, `ValidationInput`-only input, `ValidationIssue`-only output, facts-not-exceptions, immutable metadata, principled DI). The validation-layer analogue of the Normalization Stage Implementation Contract. |
