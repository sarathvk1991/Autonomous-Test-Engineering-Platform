...
| **Prompt Governance Subsystem** (CAP-071–073) | `PROMPT_METADATA_VERSION` 1.0 · `PROMPT_DEFINITION_VERSION` 1.0 | Approved — foundational | Not Frozen (stable) | Not Recorded | `requirement_intelligence/prompts/README.md` · ADR-0014 | Prompt Framework (CAP-011); Reasoning Contract | `shared/prompts/models/`, `shared/prompts/framework/` (generic mechanism, relocated additive 2026-07-27, ADR-0043 D4 carve-out-3), `requirement_intelligence/prompts/versions/` (Layer 1 content, unmoved) implemented + 124 unit tests passing | Governs prompt lifecycle, SHA-256 fingerprinting, compatibility metadata, semantic versioning rules, explicit registry, and regression contract. Architecture document is `prompts/README.md`; governed by ADR-0014. No existing frozen contract is altered. |
| **Productization Governance Contract** (CAP-070) | Governance contract 1.0 · Golden Dataset `GOLDEN_DATASET_VERSION` 1.1.0 (**versioned independently**; re-baselined at CAP-077F.2 for the grounding artifacts) · Architecture Version 1.2.0 | Approved — foundational — **FROZEN** | **Frozen** — **governance contract only** (has a Governance Contract Freeze section §13); the **golden dataset is NOT frozen** | Not Recorded | `docs/productization/golden-baseline.md` §13 | Platform Capability Matrix (§5.8 CAP-071–073) · Architecture Coverage Dashboard (§4 CAP-071–073) | Productization suite (70 tests, Phase 3–6) + golden dataset implemented + passing | Freezes the **governance contract** — the baseline's role (Release Regression Baseline), validation scope, determinism contract, ownership boundaries, and regression procedure. The **golden dataset contents are explicitly NOT frozen**: they stay independently versioned via `GOLDEN_DATASET_VERSION` and evolve additively under the doc's §7.3 re-baselining procedure, with **no ADR and no governance-contract change**. Governance registration only (CAP-070B) — no architecture, no ADR, no code change. |
