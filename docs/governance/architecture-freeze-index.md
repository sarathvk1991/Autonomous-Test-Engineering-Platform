...
| Define and govern the **first** engineering-readiness criterion deterministically (its concern, deterministic basis, verdicts, and boundary vs Validation) before implementation | **Yes — recorded as an ADR** | ADR-0013 (**Proposed**) governs **`CP1-0001` (RequirementPresenceCriterion)**: the validated response must carry **≥ 1** requirement across the governed requirement collections (`functional_/security_/quality_requirements`) — a deterministic **element count**, `CP1Input`-only, **no** semantics/NLP/LLM/policy. **PASS** (≥ 1) / **FAIL** (zero); **WARN reserved**. Draws the governed **boundary vs Validation's Business Rule layer** (`BUSINESS-0001/0003` coverage/minimum stay Reserved · Deferred, distinct — ADR-0006). Defines only *what* CP1-0001 is; **no** implementation and **no** framework/engine/composition/canonical-model change. Populates the Engineering Readiness Criteria Catalog (CAP-061) §9. |
