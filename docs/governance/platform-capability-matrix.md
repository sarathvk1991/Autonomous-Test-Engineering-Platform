...
**Artifact-Level Generation Cache (CAP-089)** is **built and tested for four of its
five target generators, three of the four also live-measured** (`LiveStepDefinitionGenerator`,
`LiveFeatureContentGenerator`, `LiveTestDataGenerator` — live-measured; `LivePageObjectGenerator`
— correctness proven deterministically, added 2026-08-20 once the page-object
live-wiring arc closed the `GenerationIdentity` prerequisite it was missing, live
measurement pending that generator's own separate CLI activation — Accepted for this
scope, governed by ADR-0050) — but it is **not wired** into any execution pipeline,
and one of the five generators its own design names remains unwrapped. Remaining
work, each with a named trigger (ADR-0050 D5 / Consequences; no dates):

1. **Extend to the one remaining generator** — `LiveUtilityGenerator` — repeating the
   same store/key/decorator pattern (D5); absent from the one real token-distribution
   measured to date, so its real share is unmeasured, not small-by-measurement.
