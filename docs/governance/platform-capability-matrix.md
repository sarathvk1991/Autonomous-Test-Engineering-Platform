...
deterministic Layer, on four of its seven target generators** (`LiveStepDefinitionGenerator`,
`LiveFeatureContentGenerator`, `LiveTestDataGenerator`, and `LivePageObjectGenerator` —
Accepted for this scope, governed by ADR-0051) — **completing eval coverage of all three of
ADR-0050's own measured/cached token sinks, and, with `LivePageObjectGenerator` now also
cached (CAP-089), completing the page-object live-wiring arc's own extensions** — but it is
**not CI-wired and not live-wired**, and three of the seven generators its own design names,
plus its entire second layer, remain out of scope. Remaining work, each with a named trigger
(ADR-0051 D5 / Consequences; no dates):

1. **Extend the same curated-eval-set + property-check pattern to the remaining three
   generators/skills** — `LiveUtilityGenerator`, `LiveFeatureRemediator`,
   `RequirementAnalysisService` — one at a time, measured,
   mirroring CAP-089's own proven extension sequence (and this harness's own four-generator
   extension so far: each generator's checks must be independently derived from its OWN real,
   already-enforced detection mechanism, never copied from a prior generator's checks — proven
   four times now, including test-data's own direct reuse of a live orchestration guard and a
   public CP3 function, and page-object's own direct reuse of a public CP4 function, not merely
   a port).
...
3. **CI/runtime integration** — a `PlatformContext` composition-root method and an
   actual CI job invoking the regression gate; none exists today for any of the four
