...
| CAP-072 | Prompt Canonical Models | The immutable information models that define what a governed prompt *is*: `PromptVersion`, `PromptLifecycle`, `PromptCompatibility`, `PromptMetadata`, `PromptDefinition` | `PROMPT_METADATA_VERSION` 1.0 · `PROMPT_DEFINITION_VERSION` 1.0 | 1.0.0 | Shared | shared contracts (`Schema`) | None recorded | Production Ready | Complete | `requirement_intelligence/prompts/models/` — five canonical models, all frozen Pydantic/dataclass instances. `PromptCompatibility` owns 5 dimensions (normalization/validation/CP1/golden-dataset/output-schema versions). `PromptMetadata` owns identity + SHA-256 + lifecycle + compatibility. `PromptDefinition` is the aggregate root. 100% unit-tested (`tests/unit/test_prompt_models.py`). |
| CAP-073 | Prompt Governance Framework | Reusable, behaviour-free governance infrastructure: `PromptRegistry` (OPEN/SEALED), `PromptLoader` (SHA-256 file verification), exception hierarchy, `build_prompt_registry()` composition root | `n/a` (framework) | 1.0.0 | Framework | Prompt Canonical Models (CAP-072); `prompts/versions/` | None recorded | Production Ready | Complete | `requirement_intelligence/prompts/framework/` — registry mirrors `ValidationRegistry` + `CP1CriterionRegistry` (explicit registration, no reflection, deterministic ordering, OPEN→SEALED); `PromptLoader` reads `versions/manifest.json`, loads `.txt` files, verifies SHA-256. `build_prompt_registry()` is the canonical composition entry point: loads `requirement_analysis_v1.0.0.txt`, verifies fingerprint, assembles `PromptDefinition`, seals registry. 100% unit-tested (`tests/unit/test_prompt_registry.py`, `test_prompt_loader.py`, `test_prompt_composition.py`). |
