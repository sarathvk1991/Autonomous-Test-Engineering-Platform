...
deterministic Layer, on two of its seven target generators** (`LiveStepDefinitionGenerator`
and `LiveFeatureContentGenerator` — Accepted for this scope, governed by ADR-0051) — but it
is **not CI-wired and not live-wired**, and five of the seven generators its own design
names, plus its entire second layer, remain out of scope. Remaining work, each with a named
trigger (ADR-0051 D5 / Consequences; no dates):

1. **Extend the same curated-eval-set + property-check pattern to the remaining five
   generators/skills** — `LivePageObjectGenerator`, `LiveUtilityGenerator`,
   `LiveTestDataGenerator`, `LiveFeatureRemediator`, `RequirementAnalysisService` — one
   at a time, measured, mirroring CAP-089's own proven extension sequence (and this
   harness's own step-def→feature-content extension: each generator's checks must be
   independently derived from its OWN real, already-enforced detection mechanism, never
   copied from a prior generator's checks).
...
3. **CI/runtime integration** — a `PlatformContext` composition-root method and an
   actual CI job invoking the regression gate; none exists today for either
