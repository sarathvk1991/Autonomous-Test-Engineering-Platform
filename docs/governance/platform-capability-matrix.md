...
| ID | Capability | Purpose | Current Version | Introduced In | Owner | Dependencies | Next Planned Milestone | Maturity | Status | Notes |
| -- | ---------- | ------- | --------------- | ------------- | ----- | ------------ | ---------------------- | -------- | ------ | ----- |
| CAP-090 | Generation Quality Eval Harness | Curate a per-generator eval set of real generation contexts, grade it with deterministic property/assertion checks, and gate on a REGRESSION against a stored baseline score — never an absolute threshold — so a silent quality-drifting model/prompt change is caught before adoption, not discovered later | `n/a` (no version constant — `eval_harness/`'s shared modules carry none) | Not Recorded (no `PLATFORM_VERSION`-linked constant; not wired into the golden baseline or any CI job) | Implementation | `GenerationIdentity` (`requirement_intelligence/llm/generation_identity.py`, pinning — the eval-score key); `suite_quality_governance/cp5/` (the deterministic check class this harness's property checks compose, at the per-artifact grain CP5 itself does not operate at); ADR-0048 Traceability Graph (`BindingCompletenessReport`, consumed read-only for the coverage-shaped check, never extended); ADR-0050 Artifact-Level Generation Cache (sibling ADR-first precedent; `atomic_write.py`'s durable-write pattern reused for the baseline store); ADR-0044 (the `StepDefinitionGenerator` Protocol this first increment targets) | Extend the same curated-eval-set + property-check pattern to the remaining six target generators/skills (`LivePageObjectGenerator`, `LiveUtilityGenerator`, `LiveTestDataGenerator`, `LiveFeatureContentGenerator`, `LiveFeatureRemediator`, `RequirementAnalysisService`); resolve the live-vs-cached LLM-in-CI open question (ADR-0051 D2); then CI/runtime integration (no `PlatformContext` composition-root method or CI job exists yet). The rubric/LLM-judge layer (Layer 2, silently-wrong-logic) remains unstarted design, not merely unbuilt code | Implementation In Progress | Accepted for Layer 1, 1 of 7 target generators (`LiveStepDefinitionGenerator`) — built, tested; not CI-wired, not live-wired | Governed by **ADR-0051 (Accepted for this scope only)**; recommended id `CAP-090`, confirmed as the next unused id in the open-ended `CAP-060…` block (§3.1) — not yet entered anywhere else. **This is a partial build, not the full seven-generator, two-layer capability ADR-0051 designs.** **The reframe (D1, the centerpiece):** the real, measured `gemini-2.5-flash` 76%-defect-rate regression this arc found (wrong Cucumber import package, a markdown code fence, a fabricated duplicate page-object class) is exactly the defect class CP5's own `compile_check`/`near_duplicate_sweep` already detect, live, deterministically — the actual gap was never detection capability, only that detection ran once, manually, ad hoc, after a model swap had already shipped, not as a curated, versioned, regression-gated eval run automatically before adoption. This harness is standing *discipline* around detection the platform mostly already had, not a new detection mechanism. **The defect-shape taxonomy (D3):** structural defects (the real ones above) → deterministic property checks, this build's own scope; coverage omissions (a required step never generated) → the traceability graph's `BindingCompletenessReport` (CAP-088), consumed not extended; silently-wrong-logic (present but implements the wrong rule) → a rubric/LLM-judge layer, explicitly deferred, not designed. **Built and tested (one generator, `LiveStepDefinitionGenerator`, 34 tests, no live LLM call anywhere in the suite):** `eval_harness/` — a curated 3-case eval set seeded from the real, currently-tracked `LoginSteps.java` corpus; three deterministic property checks (`check_valid_cucumber_import`, `check_no_markdown_fence`, `check_no_fabricated_page_object_class`), each proven to FAIL on a fixture reproducing its own real defect shape and PASS on the real clean text; scoring keyed by `GenerationIdentity`, arithmetic enforced by a `model_validator`; a regression gate (`EvalBaselineStore`/`check_regression`) proven RELATIVE, not absolute — a 25% baseline vs. a 25% candidate PASSES, a 100% baseline vs. a 75% candidate REGRESSES, either reversed under any single absolute cutoff. End-to-end proof via `StubStepDefinitionGenerator`: a clean generator's first run has no baseline (`ESTABLISHED_BASELINE`, not pass/fail) and is explicitly recorded as one; a generator standing in for a worse model (the real wrong-import defect reintroduced) is caught (`REGRESSED`) relative to that baseline. **The coverage check is built but not in the default check set** — `check_step_covered` consumes `BindingCompletenessReport` read-only (proven never to re-derive it), composable when a real corpus run's traceability graph exists, which the curated, isolated eval-set cases do not have by default. **Six generators/skills remain out of scope** (`LivePageObjectGenerator`, `LiveUtilityGenerator`, `LiveTestDataGenerator`, `LiveFeatureContentGenerator`, `LiveFeatureRemediator`, `RequirementAnalysisService`), and **the entire rubric/LLM-judge layer (Layer 2) is unstarted** — no design, not merely no code; its own open questions (which judge, how pinned, whose rubric, calibration, cost) are named in ADR-0051 D5, not answered. **Not CI-wired, not live-wired** — no `PlatformContext` composition-root method, no CI job exists; the live-vs-cached LLM-in-CI question (D2) stays exactly as open as the ADR named it. One mentor throughout (Nitin). |
...
deterministic Layer, on one of its seven target generators** (`LiveStepDefinitionGenerator`
— Accepted for this scope, governed by ADR-0051) — but it is **not CI-wired and not
live-wired**, and six of the seven generators its own design names, plus its entire
second layer, remain out of scope. Remaining work, each with a named trigger
(ADR-0051 D5 / Consequences; no dates):

1. **Extend the same curated-eval-set + property-check pattern to the remaining six
   ...
   `RequirementAnalysisService` — one at a time, measured, mirroring CAP-089's own
   proven extension sequence.
...
3. **CI/runtime integration** — a `PlatformContext` composition-root method and an
   actual CI job invoking the regression gate; none exists today for the one
