...
| CAP-049 | Reasoning Layer | Judge reasoning integrity | `n/a` (not built) | Not Recorded | Implementation | Validation Framework, ParsedResponse, Reasoning Contract | Implement `REASONING-0001…0003` | Architecture Complete | Implementable | Per ADR-0006 (as revised): **`REASONING-0002`** comparison mechanism frozen byte-exact by ADR-0008 — **implemented**. **`REASONING-0001`** (ContradictoryRequirement) is **Reserved · Deferred** by ADR-0009 (Proposed) — no governed deterministic contradiction mechanism exists (semantic/logical; superseding its ADR-0006 Implementable classification). **`REASONING-0003`** (CircularLogic) remains classified per ADR-0006 pending its own mechanism review (same semantic-mechanism gap likely applies). Semantic contradiction/coherence detection is a future capability behind a future ADR. |
