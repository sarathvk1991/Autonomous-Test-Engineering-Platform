...
| CAP-032 | ResponseNormalizer | Concrete producer of a real `ParsedResponse` (+ `NORMALIZATION-0001…0005`) | `n/a` (not built) | Not Recorded | Implementation | Normalization Framework (CAP-030), ParsedResponse (CAP-031), `LLMResponse` | **Next milestone** — build the normalizer + responsibilities | Concept | Planned | No class exists; framework and canonical model are ready for it. |
