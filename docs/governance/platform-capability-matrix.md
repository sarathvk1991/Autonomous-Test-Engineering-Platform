...
| `CAP-060…` | Downstream / Future | `CAP-060…073` | `CAP-074…` |
...
| ID | Capability | Architecture | Framework | Canonical Models | Implementation | Testing | Frozen |
| -- | ---------- | :----------: | :-------: | :--------------: | :------------: | :-----: | :----: |
| CAP-060 | CP1 Validator (subsystem) | ✓ | n/a | ✓ | ✓ | ✓ | ✗ |
...
| CAP-068 | CP1 Reporting & Execution Package Integration | ✓ | n/a | n/a | ✓ | ✓ | ✗ |

**Governance**

| ID | Capability | Purpose | Current Version | Introduced In | Owner | Dependencies | Next Planned Milestone | Maturity | Status | Notes |
| -- | ---------- | ------- | --------------- | ------------- | ----- | ------------ | ---------------------- | -------- | ------ | ----- |
| CAP-060 | CP1 Validator (Engineering Readiness subsystem) | The Engineering Readiness subsystem — the downstream quality gate consuming validated output | `n/a` (umbrella) | 1.0.0 | Implementation | Response Validator; CP1 Models (CAP-062); CP1 Framework (CAP-063) | None — subsystem complete; new criteria governed independently by CAP-061 | Production Ready | Complete | **The CP1 Engineering Readiness subsystem is complete**, architecturally governed by ADR-0011/0012 (Accepted). Every responsibility this umbrella envisioned is implemented: **canonical models** (CAP-062), **framework** (CAP-063), **Validation → CP1 seam** (CAP-064), **engine** (CAP-065), **composition root** (CAP-066), **PlatformContext & CLI integration** (CAP-067B), and **reporting/execution-package integration** (CAP-068). CP1 **executes end-to-end** (`Analysis → Normalization → Validation → ValidationToCP1Handoff → CP1Service.run() → Execution Package`) and its `CP1Result` is surfaced as `cp1_report.md`. Legacy `validators/cp1.py` stub **reconciled (CAP-063A)** and **retired (CAP-069)** — CP1's canonical entry point is `CP1Service` (`cp1/response/cp1_composition.py`); all CP1 responsibilities live in the first-class `requirement_intelligence.cp1` subsystem (ADR-0011 §D9 reconciliation). **CAP-060 tracks subsystem maturity only; the growth of individual engineering-readiness criteria is governed independently by CAP-061** (Engineering Readiness Criteria Catalog) — exactly as the frozen Validation Framework's maturity is independent of new rules added via the Validation Rule Catalog. Adding future criteria (`CP1-0002`, `CP1-0003`, …) does not change CAP-060. |
...
### 5.8 Prompt Governance

**Lifecycle**

| ID | Capability | Architecture | Framework | Canonical Models | Implementation | Testing | Frozen |
| -- | ---------- | :----------: | :-------: | :--------------: | :------------: | :-----: | :----: |
| CAP-071 | Prompt Governance Subsystem | ✓ | ✓ | ✓ | ✓ | ✓ | ✗ |
| CAP-072 | Prompt Canonical Models | ✓ | n/a | ✓ | ✓ | ✓ | ✗ |
| CAP-073 | Prompt Governance Framework | ✓ | ✓ | n/a | ✓ | ✓ | ✗ |

**Governance**

| ID | Capability | Purpose | Current Version | Introduced In | Owner | Dependencies | Next Planned Milestone | Maturity | Status | Notes |
| -- | ---------- | ------- | --------------- | ------------- | ----- | ------------ | ---------------------- | -------- | ------ | ----- |
| CAP-071 | Prompt Governance Subsystem | Elevate prompts to first-class governed assets with lifecycle, versioning, fingerprinting, compatibility, and explicit registration | `n/a` (umbrella) | 1.0.0 | Architecture | Prompt Framework (CAP-011); ADR-0014 | None recorded | Production Ready | Complete | **Prompt Governance subsystem complete**, governed by **ADR-0014 (Accepted)**. Architecture document: `requirement_intelligence/prompts/README.md`. Every responsibility envisioned is implemented: **canonical models** (CAP-072), **framework** (CAP-073). Adds governed lifecycle, SHA-256 fingerprinting, compatibility metadata, semantic versioning rules, and regression contract around the unchanged `prompt_constants.py` / `RequirementPromptBuilder`. 124 new unit tests (100% coverage). |
| CAP-072 | Prompt Canonical Models | The immutable information models that define what a governed prompt *is*: `PromptVersion`, `PromptLifecycle`, `PromptCompatibility`, `PromptMetadata`, `PromptDefinition` | `PROMPT_METADATA_VERSION` 1.0 · `PROMPT_DEFINITION_VERSION` 1.0 | 1.0.0 | Shared | shared contracts (`Schema`) | None recorded | Production Ready | Complete | Package relocated (additive, 2026-07-27, ADR-0043 D4 carve-out-3): now `shared/prompts/models/` — five canonical models, all frozen Pydantic/dataclass instances, generic infrastructure with no Layer-1-specific logic (`requirement_intelligence/prompts/` retains only Layer 1's own content — composition, versions, constants, builder). `PromptCompatibility` owns 5 dimensions (normalization/validation/CP1/golden-dataset/output-schema versions) — moved unchanged, a deferred generalization (`architecture-baseline-v2.md` §4). `PromptMetadata` owns identity + SHA-256 + lifecycle + compatibility. `PromptDefinition` is the aggregate root. 100% unit-tested (`tests/unit/test_prompt_models.py`). |
| CAP-073 | Prompt Governance Framework | Reusable, behaviour-free governance infrastructure: `PromptRegistry` (OPEN/SEALED), `PromptLoader` (SHA-256 file verification), exception hierarchy, `build_prompt_registry()` composition root | `n/a` (framework) | 1.0.0 | Framework | Prompt Canonical Models (CAP-072); `prompts/versions/` | None recorded | Production Ready | Complete | Package relocated (additive, 2026-07-27, ADR-0043 D4 carve-out-3): the generic mechanism — `shared/prompts/framework/` — registry mirrors `ValidationRegistry` + `CP1CriterionRegistry` (explicit registration, no reflection, deterministic ordering, OPEN→SEALED); `PromptLoader` reads `versions/manifest.json`, loads `.txt` files, verifies SHA-256. `build_prompt_registry()` stays Layer-1 content at `requirement_intelligence/prompts/framework/composition.py` (now importing the mechanism from `shared.prompts.*`): loads `requirement_analysis_v1.0.0.txt`, verifies fingerprint, assembles `PromptDefinition`, seals registry. 100% unit-tested (`tests/unit/test_prompt_registry.py`, `test_prompt_loader.py`, `test_prompt_composition.py`); verified byte-identical post-relocation (4820 tests unchanged, golden-baseline SHA-256 determinism passing). |
...
### 5.10 Executable Specification Engineering

**Lifecycle**

| ID | Capability | Architecture | Framework | Canonical Models | Implementation | Testing | Frozen |
| -- | ---------- | :----------: | :-------: | :--------------: | :------------: | :-----: | :----: |
| CAP-087 | Executable Specification Engineering | ✓ | ✗ | ✗ | ✗ | ✗ | ✗ |

**Governance**

| ID | Capability | Purpose | Current Version | Introduced In | Owner | Dependencies | Next Planned Milestone | Maturity | Status | Notes |
| -- | ---------- | ------- | --------------- | ------------- | ----- | ------------ | ---------------------- | -------- | ------ | ----- |
| CAP-087 | Executable Specification Engineering | Transform one execution's judged Layer 1 output, enriched by Layer 2's institutionalized Learning, into a technology-independent, renderer-agnostic Specification Model — the platform's first Layer 2.5 capability | `n/a` (no version constant exists yet — architecture only) | Not Recorded | Architecture | `RequirementEnhancementResult` (ADR-0018), `GroundingResult` (ADR-0016), `ValidationResult` (Validation architecture), `RecommendationResult` (ADR-0019), `LearningResult` (ADR-0028/ADR-0029) | **CAP-087B — Deterministic Specification Engineering Engine** | Architecture Complete | Proposed | Governed by **ADR-0030 (Proposed)**; design: `docs/proposals/executable-specification-engineering.md`; governance: `docs/governance/executable-specification-engineering-governance.md`. Introduces **Layer 2.5** (additive to ADR-0020 — see its Stage 4/9/10). No package, code, model, service, policy, or `PlatformContext` change exists yet — every canonical model (`SpecificationPlan`, `BusinessFeature`, `BusinessScenario`, `BusinessRule`, `AcceptanceCriterion`, `ScenarioStep`, `ScenarioTag`, `ScenarioOutline`, `Traceability`, `SpecificationMetrics`, `ValidationSummary`, `RendererMetadata`, `SpecificationEngineeringResult`) and the ten-collaborator internal decomposition are documentation only. Canonical output is renderer-agnostic by design; Cucumber is the first reserved renderer target. Runtime behaviour, golden baseline, and Architecture/Platform Version are all unchanged by this milestone. |

### 5.11 Traceability Graph

**Lifecycle**

| ID | Capability | Architecture | Framework | Canonical Models | Implementation | Testing | Frozen |
| -- | ---------- | :----------: | :-------: | :--------------: | :------------: | :-----: | :----: |
| CAP-088 | Traceability Graph | ✓ | n/a | ✓ | ✓ | ✓ | ✗ |
...
| ID | Capability | Purpose | Current Version | Introduced In | Owner | Dependencies | Next Planned Milestone | Maturity | Status | Notes |
| -- | ---------- | ------- | --------------- | ------------- | ----- | ------------ | ---------------------- | -------- | ------ | ----- |
| CAP-089 | Artifact-Level Generation Cache | Skip a redundant LLM call by keying each generated artifact on the exact deterministic payload its own generator serializes before calling the provider, combined with its `GenerationIdentity` — a hit replays the stored artifact and identity; a miss generates, then stores, exactly as today | `n/a` (no version constant — the shared store/key module, `generation_cache.py`, carries none) | Not Recorded (no `PLATFORM_VERSION`-linked constant; not wired into the golden baseline) | Implementation | `GenerationIdentity` (`requirement_intelligence/llm/generation_identity.py`, pinning); ADR-0044 (the four L3 generator Protocols this cache wraps or targets); ADR-0043 (`FeatureContentGenerator`, the fifth); ADR-0036's `_hash_artifacts` content-hash *pattern* (reused; its `RunStateManager` class is not, D2); ADR-0048 Traceability Graph (sibling precedent; its `change_impact_for_method`/`build_change_impact_report` is this cache's own downstream consumer once delta-scoped regeneration is built) | Runtime integration (a `PlatformContext` composition-root method — none exists for any of the five built decorators) | Implementation Complete | Accepted for 5 of 5 target generators — ALL BUILT, tested (3 of 5 also live-measured; page-object and utility proven deterministically only); not wired | Governed by **ADR-0050 (Accepted for this scope only)**; recommended id `CAP-089`, confirmed as the next unused id in the open-ended `CAP-060…` block (§3.1) — not yet entered anywhere else. **The full five-generator capability ADR-0050 designs is now built** (only the remediator stays unwrapped, D5's own explicit, permanent exclusion — not a gap). Introduces a new subsystem wrapping five LLM-driven generation seams (`LiveStepDefinitionGenerator`, `LivePageObjectGenerator`, `LiveUtilityGenerator`, `LiveTestDataGenerator`, `LiveFeatureContentGenerator`); `LiveFeatureRemediator` is explicitly excluded (D5 — repairs a prior attempt rather than performing first-generation, independently rare). **The key (D1, the centerpiece):** `sha256(prompt_id + prompt_version + prompt_sha256 + provider + model + input_payload_json)`, where `input_payload_json` is the exact string each generator's own `_build_prompt` already serializes immediately before its LLM call — corrected from the prior surfacing note's own `REQ-*`-based recommendation, checked against the real code and found to under-cover ordinary narrative/acceptance-criterion edits and to not apply at all to the four L3 generators. **The store (D2):** on-disk, content-addressed, reusing `_hash_artifacts`'s pattern and `atomic_write.py`, a new sibling module — not `RunStateManager` itself (closed to its fixed 19-entry stage catalogue). **Interception (D3):** one `Caching<X>Generator` decorator per Protocol, sharing one store module; a hit replays the *stored* `GenerationIdentity`, never a freshly-constructed one. **All five target generators are now built.** Three, each proven by dedicated unit tests and one real, live-measured run (no simulation): `CachingStepDefinitionGenerator` (`automation_engineering/generation/`, 33 tests, 7003→0 tokens across 3 real calls on re-run); `CachingFeatureContentGenerator` (`feature_engineering/generation/`, 13 tests, 3702→0 tokens — the distribution's single biggest sink, 45.4% of the 20-call sample in `docs/architecture/mentor-feedback-scoping.md`); `CachingTestDataGenerator` (`automation_engineering/generation/`, 13 tests, 3402→0 tokens — the other near-equal sink, 43.4%). **Together, feature-content and test-data alone already account for ~89% of that one measured run's own token total** (`step_definition_generation` itself recorded *zero* tokens in that specific run — 30 of 60 step-def needs were reuse hits, the other 30 escalated before reaching the generator — a fact about that run's own catalog state, not a claim about this cache). A fourth, `CachingPageObjectGenerator` (`automation_engineering/generation/`, 19 tests, added 2026-08-20 once the page-object live-wiring arc closed the `GenerationIdentity` prerequisite this generator was missing), is built and correctness-proven deterministically, but NOT yet live-measured — `page_object_generation` was absent from the one measured distribution entirely (not live-constructed in `handle_analyze` at measurement time) and remains so, since the live CLI still does not activate page-object generation at all (a separate, deliberate activation decision, not yet made). This generator's own composition (reuse-decide BINDS an existing tracked page object with no generation call at all, orthogonal to this cache which only wraps the fresh-generation call) was verified by dedicated test, not merely asserted. **A fifth and FINAL,** `CachingUtilityGenerator` (`automation_engineering/generation/`, 19 tests, added 2026-08-21 once the utility identity task closed the identical `GenerationIdentity` prerequisite for utilities), is built and correctness-proven deterministically the same way — closing the "one remaining generator" gap this row's own prior text named as the next trigger. **NOT yet live-measured, for a reason ONE STEP EARLIER than page-object's own:** utility generation is not merely absent from the one measured distribution — it is not wired into stage 15 AT ALL (`run_automation_engineering_stage` accepts no `utility_matcher`/`utility_generator` parameters, confirmed directly in the immediately preceding identity task), so its real token share stays unmeasured until BOTH stage-15 wiring AND live CLI activation happen. This generator's own composition (reuse-decide BINDS an existing tracked utility asset with no generation call at all — the SAME bind/cache orthogonality page-object already proved, confirmed a second time here) was verified by dedicated test, not merely asserted. **Not wired into any live pipeline** — `scripts/run_requirement_analysis.py` still constructs all unwrapped live generators directly; no `PlatformContext` composition-root method exists. **Relationship to delta-scoped regeneration** (the mentor cluster's own next item, `docs/architecture/mentor-feedback-scoping.md`): this cache already tells a caller an artifact is stale-or-not for its own direct inputs — it *is* delta-regen for that scope; the remaining gap is the transitive case (a dependency's output changing without the artifact's own direct payload changing), which needs the Change-Impact Graph (ADR-0048), not a new cache mechanism. One mentor throughout (Nitin). |

### 5.13 Generation Quality Eval Harness

**Lifecycle**

| ID | Capability | Architecture | Framework | Canonical Models | Implementation | Testing | Frozen |
| -- | ---------- | :----------: | :-------: | :--------------: | :------------: | :-----: | :----: |
| CAP-090 | Generation Quality Eval Harness | ✓ | ✓ | ✓ | ◑ | ✓ | ✗ |

**Governance**

| ID | Capability | Purpose | Current Version | Introduced In | Owner | Dependencies | Next Planned Milestone | Maturity | Status | Notes |
| -- | ---------- | ------- | --------------- | ------------- | ----- | ------------ | ---------------------- | -------- | ------ | ----- |
| CAP-090 | Generation Quality Eval Harness | Curate a per-generator eval set of real generation contexts, grade it with deterministic property/assertion checks, and gate on a REGRESSION against a stored baseline score — never an absolute threshold — so a silent quality-drifting model/prompt change is caught before adoption, not discovered later | `n/a` (no version constant — `eval_harness/`'s shared modules carry none) | Not Recorded (no `PLATFORM_VERSION`-linked constant; not wired into the golden baseline or any CI job) | Implementation | `GenerationIdentity` (`requirement_intelligence/llm/generation_identity.py`, pinning — the eval-score key); `suite_quality_governance/cp5/` (the deterministic check class step-def's own property checks compose); `feature_engineering/generation/assembler.py` + CP2 (feature-content's own property checks); `automation_engineering/generation/test_data_orchestrator.py`'s live SUT-binding guard + `automation_engineering/cp3/architecture.py`'s public `evaluate_long_method` (test-data's own property checks, called/ported directly, per its own 2026-08-20 increment); `automation_engineering/generation/live_page_object_generator.py`'s `_declares_method` (ported) + `automation_engineering/cp4/gate.py`'s public `evaluate_cp4` (called directly, page-object's own property checks, per its own 2026-08-21 increment); `automation_engineering/prompts/versions/generate_utilities_v1.0.0.txt`'s own OUTPUT CONTRACT/CONSTRAINTS text + `automation_engineering/cp3/architecture.py`'s public `evaluate_long_method` (utility's own property checks, ported/composed directly, per its own 2026-08-21 increment); ADR-0048 Traceability Graph (`BindingCompletenessReport`/`CompletenessReport`, consumed read-only for three generators' coverage-shaped checks, never extended); ADR-0050 Artifact-Level Generation Cache (sibling ADR-first precedent; `atomic_write.py`'s durable-write pattern reused for the baseline store); ADR-0044/ADR-0043 (the `StepDefinitionGenerator`/`FeatureContentGenerator`/`TestDataGenerator`/`PageObjectGenerator`/`UtilityGenerator` Protocols these five increments target) | Extend the same curated-eval-set + property-check pattern to the remaining two target generators/skills (`LiveFeatureRemediator`, `RequirementAnalysisService`); resolve the live-vs-cached LLM-in-CI open question (ADR-0051 D2); then CI/runtime integration (no `PlatformContext` composition-root method or CI job exists yet). The rubric/LLM-judge layer (Layer 2, silently-wrong-logic) remains unstarted design, not merely unbuilt code — and, per ADR-0051's own 2026-08-20 Investigation Note, verdicted not worth building now | Implementation In Progress | Accepted for Layer 1, 5 of 7 target generators (`LiveStepDefinitionGenerator`, `LiveFeatureContentGenerator`, `LiveTestDataGenerator`, `LivePageObjectGenerator`, `LiveUtilityGenerator`) — built, tested; not CI-wired, not live-wired. **All three of ADR-0050's own measured/cached token sinks now have quality-drift eval coverage; `LivePageObjectGenerator` and `LiveUtilityGenerator` are now both cached (CAP-089) and eval'd (CAP-090) — every one of ADR-0050's five in-scope generators now has BOTH cache AND eval, the "finish the set" arc is COMPLETE.** | Governed by **ADR-0051 (Accepted for this scope only)**; id `CAP-090`, entered in the open-ended `CAP-060…` block (§3.1). **This is a partial build, not the full seven-generator, two-layer capability ADR-0051 designs.** **The reframe (D1, the centerpiece):** the real, measured `gemini-2.5-flash` 76%-defect-rate regression this arc found (wrong Cucumber import package, a markdown code fence, a fabricated duplicate page-object class) is exactly the defect class CP5's own `compile_check`/`near_duplicate_sweep` already detect, live, deterministically — the actual gap was never detection capability, only that detection ran once, manually, ad hoc, after a model swap had already shipped, not as a curated, versioned, regression-gated eval run automatically before adoption. This harness is standing *discipline* around detection the platform mostly already had, not a new detection mechanism. **The defect-shape taxonomy (D3):** structural defects (the real ones above) → deterministic property checks, this build's own scope; coverage omissions (a required step never generated) → the traceability graph's `BindingCompletenessReport` (CAP-088), consumed not extended; silently-wrong-logic (present but implements the wrong rule) → a rubric/LLM-judge layer, explicitly deferred, not designed. **Built and tested (first generator, `LiveStepDefinitionGenerator`, 34 tests, no live LLM call anywhere in the suite):** `eval_harness/` — a curated 3-case eval set seeded from the real, currently-tracked `LoginSteps.java` corpus; three deterministic property checks (`check_valid_cucumber_import`, `check_no_markdown_fence`, `check_no_fabricated_page_object_class`), each proven to FAIL on a fixture reproducing its own real defect shape and PASS on the real clean text; scoring keyed by `GenerationIdentity`, arithmetic enforced by a `model_validator`; a regression gate (`EvalBaselineStore`/`check_regression`) proven RELATIVE, not absolute — a 25% baseline vs. a 25% candidate PASSES, a 100% baseline vs. a 75% candidate REGRESSES, either reversed under any single absolute cutoff. End-to-end proof via `StubStepDefinitionGenerator`: a clean generator's first run has no baseline (`ESTABLISHED_BASELINE`, not pass/fail) and is explicitly recorded as one; a generator standing in for a worse model (the real wrong-import defect reintroduced) is caught (`REGRESSED`) relative to that baseline. **The coverage check is built but not in the default check set** — `check_step_covered` consumes `BindingCompletenessReport` read-only (proven never to re-derive it), composable when a real corpus run's traceability graph exists, which the curated, isolated eval-set cases do not have by default. **Second generator built (2026-08-20), `LiveFeatureContentGenerator`, 32 tests, no live LLM call anywhere in the suite:** `eval_harness/feature_content_*.py` — a curated 3-case eval set seeded from three real, currently-tracked requirements in `output/latest/testable_requirement_set.json` (the same corpus the real, live-regenerated `.feature` files under `output/executions/run-20260812T064317663150Z-a20b0cc2/` were generated from); six deterministic property checks, each a direct port of one real `generate_feature_file` tag-contract validation block (five) or the governed prompt's own no-markdown-fence clause (one) — unlike step-def, grounded in the real, already-enforced CONTRACT, not a real historical INCIDENT (feature-content's own live E2E run scored 15/15 clean, 0 escalations); a seventh real `assembler.py` block (tagged-`Background:`) was investigated and deliberately NOT ported — verified empirically that a tag before `Background:` is always a hard Gherkin parse error, never a valid-but-tagged node, so that shape is already subsumed by `check_valid_gherkin_structure`. `models.py`/`scoring.py`'s `score_eval_set`/`baseline_store.py` reused verbatim, unchanged, proving the generator-agnostic design. `check_requirement_covered` (coverage-shaped, optional) consumes CAP-088's `CompletenessReport.untested_requirements` read-only, one node level up from step-def's own `check_step_covered`. End-to-end regression arc proven identically (a stray `@REQ-*` tag — the prompt's single most explicitly forbidden defect shape — standing in for a worse-model regression, since no real historical defect exists to replay). **Third generator built (2026-08-20, same day), `LiveTestDataGenerator`, 25 tests, no live LLM call anywhere in the suite — completing eval coverage of all three of ADR-0050's own measured/cached token sinks:** `eval_harness/test_data_*.py` — a curated 3-case eval set built from the SAME three real requirements feature-content's own set uses, each with the real, currently-tracked EMPTY `TestDataSpecification` every requirement in this corpus actually has (confirmed against all 20 in `output/latest/test_data_specifications.json`, and in the contract's own docstring). Five deterministic property checks: two DIRECTLY compose already-real, already-enforced mechanisms (`check_no_env_binding` ports the live, always-on `test_data_orchestrator._check_no_env_binding` guard verbatim; `check_no_long_method` calls CP3's real, public `evaluate_long_method` directly, no port needed); three are contract-grounded, no known incident (`check_no_markdown_fence`, `check_class_name_matches`, `check_no_webdriver_reference` — the last closing a REAL gap, since CP3's own `direct_webdriver_action` criterion explicitly excludes test-data's package from evaluation). One check (field-variant coverage) was considered and honestly NOT built: no real `TestDataSpecification` this platform has ever emitted carries a non-empty `fields` list to ground it against. `models.py`/`scoring.py`'s `score_eval_set`/`baseline_store.py` reused verbatim a THIRD time; `check_requirement_covered` (coverage-shaped, optional) reused VERBATIM from feature-content's own module — no new coverage module written. End-to-end regression arc proven identically (a `ConfigReader.env(...)` call — the real orchestration guard's own violation shape — standing in for a worse-model regression). **Fourth generator built (2026-08-21), `LivePageObjectGenerator`, 25 tests, no live LLM call anywhere in the suite — the RICHEST grounding of any increment in this arc:** `eval_harness/page_object_*.py` — a curated 3-case eval set seeded from the real, currently-tracked page-object catalog (`test-suite-baseline/src/test/java/com/automation/pages/`, 33 real page objects), continuing `LoginPage`'s own two methods `STEP_DEFINITION_EVAL_SET` already calls through, plus one real `InventoryPage` method for a parameter shape `LoginPage` alone does not exercise. Four deterministic property checks: three each guard the regression of one of the THREE real, MEASURED 2026-08-10 live-regeneration defects (`check_method_names_present` — 67% method-name mismatch; `check_di_constructor` — `new XPage()` vs. constructor-injected `WebDriver`, ADR-0041 D5; `check_no_fictional_basepage_helper` — 31 of 32 classes calling an invented `BasePage` method), the fourth (`check_locator_validity`) composes CP4's real, already-live `evaluate_cp4` DIRECTLY (no port needed — CP4 has no adapter seam at all), proven to fail on an absolute-XPath locator exactly as the real stage-16 wiring does. `duplicate_locators` (CP4's own cross-class criterion) structurally cannot fire at this package's one-artifact-per-case grain — an honest consequence of the scoring grain, not an unreachable check invented here; the other three CP4 criteria remain fully live. `models.py`/`scoring.py`'s `score_eval_set`/`baseline_store.py` reused verbatim a FOURTH time. No coverage-shaped check was built: a page object keys on `class_name`/`method_name`, not a `requirement_id`, so `CompletenessReport` has no node it maps onto — named honestly, not forced. End-to-end regression arc proven identically (defect 1's own dominant shape — a paraphrased method name — reintroduced into every case). **Milestone: `LivePageObjectGenerator` is now BOTH cached (CAP-089) AND eval'd (CAP-090)** — the page-object live-wiring arc's own extensions are complete. **Fifth generator built (2026-08-21, same day), `LiveUtilityGenerator`, 31 tests, no live LLM call anywhere in the suite — the "finish the set" arc COMPLETE:** `eval_harness/utility_*.py` — a curated eval set with a HYBRID, honestly-reported seed (two cases from `ConfigReader`'s own two real, currently-tracked methods, `env`/`data`, the ONLY committed utility this platform has, `target_package` supplied as its own real package `com.automation.base` rather than the `com.automation.utils` default it predates; one case built the way `orchestrate_utility_method`'s own `NoMatch` branch actually constructs a fresh context today, since no second real tracked utility exists to seed a third real-corpus case from). Five deterministic property checks — the honest count, not forced to match any prior increment's: `check_no_markdown_fence`/`check_class_name_matches` (contract-grounded, the same OUTPUT CONTRACT clause shape test-data already ported), `check_no_selenium_or_basepage_reference` (a stricter cousin of test-data's own WebDriver check — utility's contract additionally forbids extending `BasePage`), `check_no_long_method` (composes CP3's real, public `evaluate_long_method` directly), and `check_static_utility_shape` — a genuinely NEW structural check no existing CP3/CP4 criterion already covers, guarding utility's own most distinctive contract clause (final class, one private no-arg constructor, static-only methods, verified directly against a real `javalang` parse of `ConfigReader.java`). **Utility's own grounding is CONTRACT, not incident, confirmed not assumed** — utility generation has never once run live (absent from stage 15's own wiring and from the one measured live token distribution), so there is no real defect to replay; every check is grounded in the governed `generate_utilities` v1.0.0 prompt's own OUTPUT CONTRACT/CONSTRAINTS text instead. One method-parameter-shape check was considered and honestly NOT built: `UtilityGenerationContext` names no specific method the way page-object's `method_name` does, so there is no real anchor to check parameter correspondence against. `models.py`/`scoring.py`'s `score_eval_set`/`baseline_store.py` reused verbatim a FIFTH time. No coverage-shaped check was built, the identical reason page-object's own increment gave (a utility keys on `class_name`, not a `requirement_id`). End-to-end regression arc proven identically (a markdown code fence — the prompt's own single most explicit forbidden defect shape, mirroring feature-content's identical choice — standing in for a worse-model regression, since no real historical defect exists to replay). **THE MILESTONE: `LiveUtilityGenerator` is now BOTH cached (CAP-089) AND eval'd (CAP-090)** — every one of ADR-0050's five in-scope generators (`LiveStepDefinitionGenerator`, `LiveFeatureContentGenerator`, `LiveTestDataGenerator`, `LivePageObjectGenerator`, `LiveUtilityGenerator`) now has BOTH cache AND eval; no in-scope generator has neither — the identity→cache→eval "finish the set" arc this task pursued across its own three steps is COMPLETE. **Two generators/skills remain out of scope** (`LiveFeatureRemediator`, `RequirementAnalysisService`), and **the entire rubric/LLM-judge layer (Layer 2) remains unbuilt, now with its value INVESTIGATED (2026-08-20) — NOT WORTH BUILDING NOW**: silently-wrong-logic (the judge's only real residual scope once Layer 1 + CAP-088 are subtracted) is unobserved across every real corpus this arc has produced; a judge's own reliability is uncalibratable without ground truth this platform does not have; and ADR-0049's Engineering Constitution Article VII ("Deterministic Gates Decide") means a judge could never GATE on this platform regardless — only ever advisory. Full analysis in ADR-0051's own "Investigation Note" (2026-08-20); trigger to reopen: a real observed silently-wrong-logic instance, named not scheduled. **Not CI-wired, not live-wired** — no `PlatformContext` composition-root method, no CI job exists; the live-vs-cached LLM-in-CI question (D2) stays exactly as open as the ADR named it. Utility's own eval score, like its cache sibling, stays unmeasured against any real corpus until utility generation is BOTH wired into stage 15 AND activated in the live CLI — two separate, deliberate decisions, neither made by this increment. One mentor throughout (Nitin). |

### 5.14 Cross-Corpus Requirement Completeness

**Lifecycle**

| ID | Capability | Architecture | Framework | Canonical Models | Implementation | Testing | Frozen |
| -- | ---------- | :----------: | :-------: | :--------------: | :------------: | :-----: | :----: |
| CAP-091 | Cross-Corpus Requirement Completeness | ✓ | n/a | ✓ | ✓ | ✓ | ✗ |

**Governance**

| ID | Capability | Purpose | Current Version | Introduced In | Owner | Dependencies | Next Planned Milestone | Maturity | Status | Notes |
| -- | ---------- | ------- | --------------- | ------------- | ----- | ------------ | ---------------------- | -------- | ------ | ----- |
| CAP-091 | Cross-Corpus Requirement Completeness | Assess whether one execution's requirement set is anomalously incomplete relative to the distribution of requirement counts this platform's own historical executions have produced — both mentors' #1 strategic risk ("house of cards": an incomplete input corpus, undetected, makes every downstream completeness check complete-against-an-incomplete-spec) | `n/a` (no version constant — minimal-slice build, no version-axis ceremony, mirroring `CAP-088`) | Not Recorded (no `PLATFORM_VERSION`-linked constant; not wired into the golden baseline or any live pipeline) | Implementation | `TestableRequirementSet` (ADR-0034/ADR-0042, the artifact read across many executions); the Historical Dataset arc's piece 2/3 extraction pattern (mirrored, never imported); ADR-0023 §D9/§D10 (the frozen boundary respected: `HistoricalExecutionRecord` never crosses the `knowledge_graph` package boundary, so this capability builds its own, disjoint reader) | Runtime integration (a `PlatformContext` composition root + live pipeline wiring so a real, in-flight execution's own count gets assessed automatically) — none yet designed, mirroring `CAP-088`'s own identical next-milestone framing | Implementation Complete | **Accepted — built, tested, measured end-to-end on real data; not wired** | Governed by **ADR-0052 (Accepted, 2026-08-25)**; recommended id `CAP-091`, confirmed as the next unused id in the open-ended `CAP-060…` block (§3.1) — not yet entered anywhere else. **Not freeze-blocked**: ADR-0032's freeze is scoped to the `CAP-001…073`/`CAP-081…086` ranges only (its own Decision text, quoted in ADR-0052 D4); the open `CAP-060…` block already hosts three precedents with the identical "placement left explicitly open" posture (`CAP-087`, `CAP-088`, and — same block, different posture — `CAP-089`/`CAP-090`). **Placement in the platform's layer model is explicitly left open** (ADR-0052 D4), mirroring `CAP-088`'s own still-open placement (ADR-0048 D2) — reads Runtime Truth (`TestableRequirementSet`) across many executions without a `HistoricalDatasetReference`-shaped indirection, by deliberate design (ADR-0052 D3, mirroring ADR-0048 D6's identical rejection of a synthetic stand-in). **Granularity is per-run-total only, a recorded limitation, not a defect** (ADR-0052 D2): `component`/`functionalTag` were checked directly against the real corpus and found constant within every execution, so a hoped-for per-component comparison collapses to the same signal as the coarser per-run total; real signal exists at the run-total level (requirement counts cluster at 15/20/30 across the 13 real qualifying executions, checked directly, not assumed). **Piece 1 (2026-08-25): the reader.** `requirement_intelligence/corpus_completeness/` — `CorpusExecutionReader`/`CorpusExecutionRecord` (own, disjoint types) enumerate `output/executions/` and extract each qualifying run's requirement count plus its representative `component`/`functionalTag`, at the dict level (never `model_validate`), mirroring the Historical Dataset arc's own file-based-provider pattern (piece 2/3) without importing it. **Piece 2 (2026-08-25, same day): the comparison engine.** `CorpusCompletenessEngine`/`CompletenessAssessment`/`HistoricalDistributionSummary` read THROUGH piece 1's reader and assess a given run's requirement count against the historical distribution. **The one real design decision, made and recorded, not assumed:** the anomaly rule is BELOW-HISTORICAL-MINIMUM, chosen over a z-score/stddev model after checking the real distribution's actual shape — 13 points, trimodal (15×3/20×7/30×3), mean 21.15, stddev 5.25 — under which one stddev below the mean (~15.9) sits ABOVE the real, legitimate, three-times-repeated 15-cluster, so a naive z-score rule would misflag a normal historical value; below-minimum makes no shape assumption and never flags an already-observed count. Recorded as a genuine judgment call, not fully data-forced, and low-risk to revisit since D5 keeps this report-only (nothing gates on it). **Cold-start honesty:** below `MIN_SAMPLE_SIZE_FOR_ASSESSMENT` (3 — the smallest cluster size, 15's and 30's, the real corpus has ever produced) the engine returns `INSUFFICIENT_HISTORY`, never a fabricated verdict. **Piece 3 (2026-08-25, same day, final): the report.** `CorpusCompletenessReport`/`build_corpus_completeness_report` — a pure, deterministic projection of piece 2's assessment into D5's own vocabulary (`outlier` for `flagged`, `rationale` for `reason`), mirroring `CAP-088`'s own `CompletenessReport` and CP7's whole-suite report: no `overall_verdict`/`passed`/gate field, proven both by `__dataclass_fields__` introspection and by confirming no gate/block/fail/raise-shaped method exists anywhere on the type. `output/executions/` → `CorpusExecutionReader` → `CorpusCompletenessEngine` → `CorpusCompletenessReport` now runs end-to-end, proven against the REAL corpus, not only fixtures: n=13, min=15/median=20/max=30; the real minimum (15) and a normal count (20) both correctly surface `outlier=False`; an anomalously low hypothetical count (5) correctly surfaces `outlier=True` with a transparent rationale and full 13-execution provenance; a count above the real maximum (35) correctly stays `outlier=False` (D1's scope is incompleteness only, never widened into general outlier detection). 49 total unit tests across the three pieces (22 reader + 16 engine + 11 report), all fixture-based, never the real gitignored corpus in committed tests; zero `knowledge_graph` imports anywhere in the package, proven by grep and by a dedicated AST-based import check per module (ADR-0052 D3 / ADR-0023 §D9/§D10 held). Report-only throughout, structurally, not by convention (ADR-0052 D5, mirroring `CAP-088`'s own D5 and CP7's rating-gating sequencing). Explicitly excludes internal-consistency checking (checked against the real corpus and found currently signal-less — every requirement has exactly one acceptance criterion, `tracesTo` is an identical 50-item list on every requirement, zero duplicate `contentHash`es — reserved as a separate, future, freeze-free slice) and subset-not-everything prioritization (a genuinely different, Layer-1-internal problem, still freeze-blocked) — both named, neither designed, in ADR-0052 D6. **Not wired into any execution pipeline** — no `PlatformContext` method, no golden-baseline entry, no Execution Package artifact; a real, in-flight execution's own count is not yet assessed automatically, mirroring exactly how `CAP-088`'s own traceability graph remains built-but-unwired. Written architecture-first, explicitly not repeating `CAP-088`'s own build-then-ADR governance debt (ADR-0048 D5). |
...
| ---- | ---------- | ------ |
| **Architecture capability catalogue** | `platform_metadata.ARCHITECTURE_COMPONENTS` with `available=True` | **11 of 14** components available (78.6%). Not available: CP1 Validator, Feature Generator, Test Generator. _(The `available` flag is a code-level display toggle in `platform_metadata`, separate from governance maturity: the CP1 Validator **subsystem** (CAP-060) is implementation-complete — see §5.6 — while its runtime `available` flag is flipped in a later code change, outside this governance record.)_ |
...
| **Completed (Production Ready)** | Ingestion & Core (CAP-001…003), AI Generation implementation (CAP-011…014), Execution & Platform (CAP-020…024), Response Normalization subsystem (CAP-030), ParsedResponse (CAP-031), ResponseNormalizer (CAP-032), **Response Validator (CAP-041, wired end-to-end incl. persistence + reporting)**, Syntax Layer (CAP-043), ValidationInput (CAP-051), **Validation Profiles (CAP-052)**, **CP1 Canonical Models (CAP-062)**, **CP1 Framework (CAP-063)**, **Validation → CP1 Seam (CAP-064)**, **CP1 Engine (CAP-065)**, **CP1 Composition Root (CAP-066)**, **CP1 PlatformContext & CLI Wiring (CAP-067B)**, **CP1 Reporting & Execution Package Integration (CAP-068)**, **CP1 Validator / Engineering Readiness subsystem (CAP-060 — umbrella complete)**, **Productization (CAP-070 — Release Regression Baseline; verification only)**, **Prompt Governance Subsystem (CAP-071 — umbrella complete, ADR-0014)**, **Prompt Canonical Models (CAP-072)**, **Prompt Governance Framework (CAP-073)**. |
...
**Artifact-Level Generation Cache (CAP-089)** is **built and tested for ALL FIVE of its
target generators — the set is complete — three of the five also live-measured**
(`LiveStepDefinitionGenerator`, `LiveFeatureContentGenerator`, `LiveTestDataGenerator` —
live-measured; `LivePageObjectGenerator` — correctness proven deterministically, added
2026-08-20 once the page-object live-wiring arc closed the `GenerationIdentity`
prerequisite it was missing, live measurement pending that generator's own separate CLI
activation; `LiveUtilityGenerator` — correctness proven deterministically, added
2026-08-21 once the utility identity task closed the identical prerequisite, live
measurement pending BOTH stage-15 wiring AND CLI activation, neither of which exists yet
— Accepted for this scope, governed by ADR-0050) — but it is **not wired** into any
execution pipeline. Remaining work, each with a named trigger (ADR-0050 D5 /
Consequences; no dates):

1. **Runtime integration** — a `PlatformContext` composition-root method and live
   pipeline wiring for all five wrapped generators; `scripts/run_requirement_analysis.py`
   still constructs each live generator unwrapped. For utility specifically, this also
   requires stage-15 wiring first (utility generation has no `AssetRecord`/stage
   presence at all today — a separate, larger gap than page-object's own "just needs
   CLI activation" state).
2. **Delta-scoped regeneration** (the mentor cluster's own next item) — consumes
   ...
   does not cover.
3. **Serialization-drift discipline** (D1's own named residual risk) — a
...
4. **`LiveFeatureRemediator`** — explicitly excluded (D5): repairs a prior attempt
...
**Generation Quality Eval Harness (CAP-090)** is **built and tested for its first,
deterministic Layer, on five of its seven target generators** (`LiveStepDefinitionGenerator`,
`LiveFeatureContentGenerator`, `LiveTestDataGenerator`, `LivePageObjectGenerator`, and
`LiveUtilityGenerator` — Accepted for this scope, governed by ADR-0051) — **completing eval
coverage of all three of ADR-0050's own measured/cached token sinks, and, with
`LivePageObjectGenerator` and `LiveUtilityGenerator` now also cached (CAP-089), completing the
"finish the set" arc: every one of ADR-0050's five in-scope generators now has BOTH cache AND
eval** — but it is **not CI-wired and not live-wired**, and two of the seven generators its own
design names, plus its entire second layer, remain out of scope. Remaining work, each with a
named trigger (ADR-0051 D5 / Consequences; no dates):

1. **Extend the same curated-eval-set + property-check pattern to the remaining two
   generators/skills** — `LiveFeatureRemediator`, `RequirementAnalysisService` — one at a
   time, measured, mirroring CAP-089's own proven extension sequence (and this harness's own
   five-generator extension so far: each generator's checks must be independently derived from
   its OWN real, already-enforced detection mechanism, never copied from a prior generator's
   checks — proven five times now, including test-data's own direct reuse of a live
   orchestration guard and a public CP3 function, page-object's own direct reuse of a public
   CP4 function, and utility's own genuinely new structural check plus a direct CP3 reuse, not
   merely a port).
2. **Resolve the live-vs-cached LLM-in-CI question** (ADR-0051 D2, explicitly left
   open) — whether a CI run makes a real LLM call per eval case or replays a
   pinned/cached response set (the artifact-generation cache, ADR-0050, is one
   candidate mechanism); this determines the gate's own cadence.
3. **CI/runtime integration** — a `PlatformContext` composition-root method and an
   actual CI job invoking the regression gate; none exists today for any of the five
   generators already built.
4. **The rubric/LLM-judge layer (Layer 2, for silently-wrong-logic)** — named as
   deferred future scope by ADR-0051 D5, with its own open questions (which judge,
   how is it pinned, whose rubric, how is it calibrated, at what cost) explicitly
   unresolved; this is unstarted *design*, not merely unbuilt code, and is not
   scheduled by naming it here.

None of these require a redesign of the built capability when taken up — extending
to another generator repeats the same curated-set/property-check/scoring pattern
(ADR-0051 D2); the judge layer is an additive second layer, not a replacement of
Layer 1.
