...
| ID | Capability | Architecture | Framework | Canonical Models | Implementation | Testing | Frozen |
| -- | ---------- | :----------: | :-------: | :--------------: | :------------: | :-----: | :----: |
| CAP-091 | Cross-Corpus Requirement Completeness | ✓ | ✗ | ◑ | ◑ | ✓ | ✗ |

**Governance**

| ID | Capability | Purpose | Current Version | Introduced In | Owner | Dependencies | Next Planned Milestone | Maturity | Status | Notes |
| -- | ---------- | ------- | --------------- | ------------- | ----- | ------------ | ---------------------- | -------- | ------ | ----- |
| CAP-091 | Cross-Corpus Requirement Completeness | Assess whether one execution's requirement set is anomalously incomplete relative to the distribution of requirement counts this platform's own historical executions have produced — both mentors' #1 strategic risk ("house of cards": an incomplete input corpus, undetected, makes every downstream completeness check complete-against-an-incomplete-spec) | `n/a` (no version constant exists yet — no governed artifact this capability emits has shipped) | Not Recorded | Architecture; Implementation (reader + comparison engine, report not started) | `TestableRequirementSet` (ADR-0034/ADR-0042, the artifact read across many executions); the Historical Dataset arc's piece 2/3 extraction pattern (mirrored, never imported); ADR-0023 §D9/§D10 (the frozen boundary respected: `HistoricalExecutionRecord` never crosses the `knowledge_graph` package boundary, so this capability builds its own, disjoint reader) | **CAP-091C — `CorpusCompletenessReport`** (a future, separate implementation milestone; the reader and comparison engine it renders are now both built) | Foundation (reader + engine implemented; report not started) | Proposed | Governed by **ADR-0052 (Proposed)**; recommended id `CAP-091`, confirmed as the next unused id in the open-ended `CAP-060…` block (§3.1) — not yet entered anywhere else. **Not freeze-blocked**: ADR-0032's freeze is scoped to the `CAP-001…073`/`CAP-081…086` ranges only (its own Decision text, quoted in ADR-0052 D4); the open `CAP-060…` block already hosts three precedents with the identical "placement left explicitly open" posture (`CAP-087`, `CAP-088`, and — same block, different posture — `CAP-089`/`CAP-090`). **Placement in the platform's layer model is explicitly left open** (ADR-0052 D4), mirroring `CAP-088`'s own still-open placement (ADR-0048 D2) — reads Runtime Truth (`TestableRequirementSet`) across many executions without a `HistoricalDatasetReference`-shaped indirection, by deliberate design (ADR-0052 D3, mirroring ADR-0048 D6's identical rejection of a synthetic stand-in). **Granularity is per-run-total only, a recorded limitation, not a defect** (ADR-0052 D2): `component`/`functionalTag` were checked directly against the real corpus and found constant within every execution, so a hoped-for per-component comparison collapses to the same signal as the coarser per-run total; real signal exists at the run-total level (requirement counts cluster at 15/20/30 across the 13 real qualifying executions, checked directly, not assumed). **Piece 1 built (2026-08-25): the reader.** `requirement_intelligence/corpus_completeness/` — `CorpusExecutionReader`/`CorpusExecutionRecord` (own, disjoint types) enumerate `output/executions/` and extract each qualifying run's requirement count plus its representative `component`/`functionalTag`, at the dict level (never `model_validate`), mirroring the Historical Dataset arc's own file-based-provider pattern (piece 2/3) without importing it. 22 unit tests (fixture-based, never the real gitignored corpus) plus a live read against the real 13-qualifying-execution corpus, confirming the 15/20/30 cluster D1 named. A grep across the package plus an AST-based import check both confirm zero `knowledge_graph` imports (ADR-0052 D3 / ADR-0023 §D9/§D10 held). **Piece 2 built (2026-08-25, same day): the comparison engine.** `CorpusCompletenessEngine`/`CompletenessAssessment`/`HistoricalDistributionSummary` (own types) read THROUGH piece 1's reader and assess a given run's requirement count against the historical distribution. **The one real design decision, made and recorded, not assumed:** the anomaly rule is BELOW-HISTORICAL-MINIMUM, chosen over a z-score/stddev model after checking the real distribution's actual shape — 13 points, trimodal (15×3/20×7/30×3), mean 21.15, stddev 5.25 — under which one stddev below the mean (~15.9) sits ABOVE the real, legitimate, three-times-repeated 15-cluster, so a naive z-score rule would misflag a normal historical value; below-minimum makes no shape assumption and never flags an already-observed count. Recorded as a genuine judgment call, not fully data-forced, and low-risk to revisit since D5 keeps this report-only (nothing gates on it yet). **Cold-start honesty:** below `MIN_SAMPLE_SIZE_FOR_ASSESSMENT` (3 — the smallest cluster size, 15's and 30's, the real corpus has ever produced) the engine returns `INSUFFICIENT_HISTORY`, never a fabricated verdict; the real 13-run corpus clears that bar. 16 new unit tests plus a live real-corpus assessment (n=13, min=15/median=20/max=30, a real hypothetical low count correctly flagged, the real min/normal/above-max counts correctly not flagged) confirm the engine on real data. Zero `knowledge_graph` imports (grep + AST check, mirroring piece 1). **Not yet built: `CorpusCompletenessReport` (D5)** — the reader and engine are enumerate/extract/assess only; nothing renders or serializes an assessment into the ADR-0052-sketched report shape yet. Report-only by design in this ADR's own scope — no gating logic designed (ADR-0052 D5, scores-first, mirroring `CAP-088`'s own D5 and CP7's rating-gating sequencing); structurally confirmed (`CompletenessAssessment` has no gate/block/fail/raise-shaped method). Explicitly excludes internal-consistency checking (checked against the real corpus and found currently signal-less — every requirement has exactly one acceptance criterion, `tracesTo` is an identical 50-item list on every requirement, zero duplicate `contentHash`es — reserved as a separate, future, freeze-free slice) and subset-not-everything prioritization (a genuinely different, Layer-1-internal problem, still freeze-blocked) — both named, neither designed, in ADR-0052 D6. Written architecture-first, explicitly not repeating `CAP-088`'s own build-then-ADR governance debt (ADR-0048 D5). |
