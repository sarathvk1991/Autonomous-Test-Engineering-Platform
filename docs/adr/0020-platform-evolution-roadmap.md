...
- **CAP-086 — Learning Framework** — Proposed, architecture frozen (ADR-0029; CAP-086A architecture & governance freeze; CAP-086A.1 engine architecture refinement & governance freeze). The fourth and final Layer 2 capability, consuming Organizational Memory's completed result — the first Layer 2 capability to consume exactly one already-completed Layer 2 tier rather than a Historical Dataset reference or a two-peer fan-in; the future engine's 12-collaborator pipeline and internal governance are now pre-specified; still unwired into any execution pipeline.
