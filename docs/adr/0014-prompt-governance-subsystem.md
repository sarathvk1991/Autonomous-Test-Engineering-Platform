...
**Relocation note (additive, 2026-07-27, ADR-0043 D4 carve-out-3).** The forcing clause above fired during Layer 2's prompt-conversion work and was resolved as an ADR-0032 carve-out-3 reading (a no-behavior-change rename/relocation, not new Layer 1 judgement capability — no freeze lift required). The generic framework mechanism relocated to `shared.prompts.framework` (`PromptRegistry`, `PromptLoader`, the exception hierarchy, the governed template contract) and `shared.prompts.models` (`PromptVersion`, `PromptLifecycle`, `PromptCompatibility`, `PromptMetadata`, `PromptDefinition`). Two elements moved **unchanged**, deliberately: `ARTIFACT_CONTEXT_PLACEHOLDER = "{artifact_context}"` and `PromptCompatibility`'s five Layer-1-named fields (`normalization_version`/`validation_version`/`cp1_version`/`golden_dataset_version`/`output_schema_version`) — both are Layer-1-shaped; generalizing either is a separate, later decision, not made by this relocation (recorded as open items in `architecture-baseline-v2.md` §4). Layer 1's own **content** stayed exactly where it was: `composition.py` (now importing the mechanism from `shared.prompts.*`, the same way Layer 2 will), `versions/`, `prompt_constants.py`, `prompt_templates.py`, `requirement_prompt_builder.py`, and this package's own top-level `__init__.py` (same re-export surface, repointed internally). Verified byte-identical behavior: 4820 tests passing before and after (unchanged count), the golden-baseline productization suite's `test_determinism_prompt_sha256` passing, and `PromptLoader`'s SHA-256 verification against the untouched `versions/manifest.json` still succeeding for both registered versions. No architectural invariant A–H changes; this note records where the mechanism landed, closing the relocation side of the amendment note above. Governance registers (`platform-capability-matrix.md` CAP-072/073, `architecture-freeze-index.md`) corrected additively in the same change.
