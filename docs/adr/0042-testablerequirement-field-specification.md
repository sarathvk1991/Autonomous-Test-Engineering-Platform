...
**Sharpening note (additive, 2026-07-28).** The paragraph above (and the correction note before it) describe `TestableRequirement.traces_to`'s granularity as "requirement→group" — populated from "`source_consolidated_id`'s `SourceArtifact` set." That description is coarser in reality than its wording implies. Verified directly against the shipped emitter, `requirement_intelligence/testable_requirement/emitter.py::_source_refs` (and re-confirmed empirically by a Layer 1 input-fixture rebuild task that ran a live analysis end to end): `traces_to` is built from `EngineeringContext.evidence.{functional,security,quality}_artifacts` — the orchestrator's full, deduplicated evidence set spanning **every contributing consolidation group in the run**, not the one group that produced a given requirement's own statement. The practical consequence, confirmed on a live run (30 requirements, 25 deduplicated `SourceRef`s): **every `TestableRequirement` in a `TestableRequirementSet` carries the identical `traces_to` array.** The true granularity is **requirement→run**, not requirement→group — coarser than either this ADR's own prose or the emitter module's docstring (which frames the scope correction only as "broader than the primary group") fully states. Two consequences follow, neither requiring a schema or Decision change (the field's *shape* — `SourceRef[]` — is unaffected, only its honest granularity description is corrected): (1) **no per-requirement source attribution is available at any granularity finer than the run** — a future Layer 7 (or any consumer) must not read `traces_to` as "the source(s) that produced this specific requirement" and must not be designed to expect that link; the finest honest granularity `contract_version` 1.0.0 offers is "every source the run's reasoning touched." (2) **In a run that mixes real and fixture-provenance sources** (e.g. authored JIRA requirements alongside representative Sonar/ZAP fixture findings), *every* requirement's `traces_to` — including JIRA-only-derived ones — contains the fixture `SourceRef`s too, once any fixture artifact reaches the run's evidence at all. Fixture-vs-real provenance for a given requirement must therefore be judged by the requirement's own **content** (title/statement), never by filtering its `traces_to`, which carries the whole run's evidence regardless of which source actually motivated that requirement's text.
