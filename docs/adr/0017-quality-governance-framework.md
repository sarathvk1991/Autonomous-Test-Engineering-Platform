...
- **Runtime status:** **Active (CAP-080D), boundary hardened (CAP-080D.1).** The full subsystem — Rule Evaluation (CAP-080B), Assessment (CAP-080B.1), Decision (CAP-080B.2), and the `DefaultQualityGovernanceService` orchestration (CAP-080C) — is wired into the live Requirement Intelligence pipeline immediately after CP1, as the terminal release authority (§D30). It consumes only `GroundingResult`, `ValidationResult`, `CP1Result`; the Execution Package projects its `QualityGovernanceResult` into three artifacts and references them from the manifest by name only — the canonical `QualityDecision` is never duplicated onto the manifest (§D31); the golden dataset remains at `1.2.0`. The Architecture Version remains **1.2.0** and no frozen contract changed.
...
- **Execution Writer & manifest.** When `quality_governance_result` is present, the writer conditionally appends `quality_governance_result.json`, `quality_governance_report.md`, and `quality_governance_summary.md` (the existing conditional-append pattern; no new flow). Those three artifacts flow into `manifest.generatedArtifacts` through the same checksum mechanism as every other file — the manifest schema is unchanged (`manifestSchemaVersion` stays `1.0.0`). Additive, CP1-pattern manifest keys (`qualityGovernanceExecuted`, `qualityGovernanceReport`, `qualityGovernanceSummary`) reference the three artifacts; when governance did not run, no key is added. **Superseded by §D31:** the original CAP-080D cut also added a fourth key, `qualityGovernanceDecision`, surfacing the verdict directly on the manifest. CAP-080D.1 removed it — see §D31 for why.

- **Release authority (Recommendation 6).** `QualityDecision` is the repository's **only** release authority. The canonical decision is read **verbatim** from the recorded `QualityDecision` and rendered nowhere but `quality_governance_result.json` / `quality_governance_report.md` / `quality_governance_summary.md` (§D31); no CLI logic, execution writer, serializer, manifest builder, release script, or downstream tool recomputes, reinterprets, overrides, or duplicates it. They consume it only, from its one canonical location.
...
### D31 — Manifest Purity & Runtime Boundary (CAP-080D.1)

CAP-080D wired the runtime in and, in the same cut, added a manifest key — `qualityGovernanceDecision` — that read the verdict verbatim and rendered it a second time on `manifest.json`. That field passed every CAP-080D test (the value was always correct, because it was copied from the one correct source) but it was the one seam in the milestone that did not match the Grounding precedent: `manifest.json` never gained a `groundingScore` or a `groundingVerdict` key, only `engineeringContextArtifact`-style references. CAP-080D.1 is a pure hardening pass — no engine, model, policy, or signature changes anywhere in the subsystem — that removes the one field that broke the pattern and freezes the boundary permanently so it cannot regrow.

**Runtime ownership (frozen).** `QualityGovernanceResult` is the sole owner of every governance runtime concept: `QualityDecision`, the overall quality score, governance findings, the assessment, the decision explanation, recommendations, and release readiness. Each exists in exactly **one** canonical runtime location — the result object itself, and its one serialization, `quality_governance_result.json`. No execution artifact, report, summary, or manifest may hold a second copy of any of them. `quality_governance_report.md` and `quality_governance_summary.md` render the same values in prose/table form for humans, but they are projections *of* the one runtime object, not alternate sources of it (D3, D30 Serializer boundary) — the audit trail still terminates at one place.

**Manifest ownership (frozen).** `manifest.json` owns exactly six kinds of information, repository-wide, and nothing else: package identity, package metadata, execution metadata, the generated-artifact inventory, checksums, and version information. Concretely, for Quality Governance that means the manifest may carry:

- `qualityGovernanceExecuted` — did the stage run at all (execution metadata, boolean).
- `qualityGovernanceReport` / `qualityGovernanceSummary` — the filenames of two of the three governed artifacts (artifact inventory; the third, `quality_governance_result.json`, is already named by `generatedArtifacts` like every other file).

The manifest may **never** carry `qualityGovernanceDecision`, a `qualityGovernanceScore`, a `qualityGovernanceDecisionVersion`, governance findings, or recommendations — those answer *what was decided*, not *what package is this / what does it contain*, and so belong exclusively to `QualityGovernanceResult`. This is permanent, not merely the current state: it applies to every future manifest field, governance or otherwise. A manifest field is legitimate only if it answers "what package is this?" or "what artifacts does it contain?" (Recommendation 1, restated here as the manifest's charter). The manifest must never become a runtime contract, a governance model, a release model, a quality model, or a reporting model — it is, and stays, an index.

**Serialization invariant (unchanged, restated).** Every governance execution artifact remains a pure projection of `QualityGovernanceResult`, reproducible from it alone, computing nothing (D30 Serializer boundary, ADR-0016 §D16's rule extended to Quality Governance). CAP-080D.1 changes no line of `QualityGovernanceSerializer` — the serializer was never the problem; the manifest builder's one extra field was.

**Execution Package responsibility (frozen).** The Execution Package — `ExecutionWriter`, `ManifestBuilder`, and the governance `QualityGovernanceSerializer` together — owns only serialization, formatting, packaging, indexing, and checksums. It must never evaluate a rule, assess an outcome, classify a failure, decide a verdict, compute a metric, compute a summary, compute a recommendation, or invoke a policy or runtime service. Everything any artifact renders must already exist, verbatim, inside `QualityGovernanceResult`. Enforced by containment tests (`tests/unit/test_manifest_purity_boundary.py`): `ManifestBuilder`, `ExecutionData`, and `ExecutionWriter` import no governance evaluator, assessment engine, decision engine, pipeline, builder, policy, or service module — `ExecutionWriter` may import only the projection-only `quality_governance.serialization` submodule, and `ManifestBuilder` imports no governance module at all (it reads `data.quality_governance_result`, typed `Any`, through exactly the fields it needs).

**One-way dependency (frozen, permanent).**

```
Engineering Context → Analysis → Grounding → Validation → CP1
        → Quality Governance → QualityGovernanceResult
        → Execution Package → manifest.json
```

`manifest.json` never flows back into the runtime, never computes a decision, and the Execution Package never reaches into a runtime *implementation* (evaluator, assessment engine, decision engine, pipeline, service) — only into the frozen `QualityGovernanceResult` contract and the projection-only serializer built on it. Everything downstream of `QualityGovernanceResult` consumes; nothing downstream computes.

**Single source of truth (audited, CAP-080D.1).** Auditing the repository confirms each governance concept has exactly one canonical runtime location and no duplication:

| Concept | Canonical location |
| --- | --- |
| `QualityDecision` | `QualityGovernanceResult.assessment.decision` |
| Overall quality score | `QualityGovernanceResult.assessment.summary.overall_quality_score` |
| Governance findings | `QualityGovernanceResult.assessment.findings` |
| Assessment | `QualityGovernanceResult.assessment` |
| Release readiness | `QualityGovernanceResult.assessment.decision` (the same `QualityDecision`, not a second field) |

Before CAP-080D.1, `QualityDecision` had a second, redundant location — `manifest.json → qualityGovernanceDecision` — that this milestone removed. No other duplication was found across the Execution Package, CLI, or any other subsystem.

**Recommendations — a gap, not a duplication.** The audit surfaced one governance concept with **no** canonical location on `QualityGovernanceResult` at all, rather than a duplicated one. `QualityDecisionResult.explanation.recommendations` (a structured `tuple[str, ...]`, D28) is produced by the Decision layer but `QualityGovernanceResultBuilder.build` copies only `quality_decision_result.summary.verdict` (prose) onto `QualitySummary.verdict` — the structured `recommendations` tuple is not propagated and `QualityDecisionResult` itself is never serialized as an artifact. This is not new: D27 already reserved a future structured recommendation model for `QualityAssessmentResult` without implementing it, and the Assessment layer's own `emit_recommendations` clause (D26) is folded into `summary_text`, not carried forward either. CAP-080D.1 changes nothing here — introducing a `QualityGovernanceResult.recommendations` field would be new functionality, which this milestone is scoped to exclude — but records the gap so a future milestone closes it in the runtime contract, never by adding a manifest field.

**Future consumers (Recommendation 4, restated permanently).** CI/CD, dashboards, release tooling, governance reports, and compliance tooling must integrate against `quality_governance_result.json` — the canonical runtime contract — never against an expanded `manifest.json`. The manifest stays intentionally minimal by design, not by oversight; growing it to save a consumer one file read is exactly the drift this section exists to forbid. A future governance capability (organization-specific policy, a compliance pack, AI-assisted governance, a regulatory profile, historical governance intelligence) is added by extending `QualityGovernanceResult` and the governance subsystem — never by adding a manifest key.

**Golden regression boundary (confirmed, unchanged).** The golden suite's determinism tests already compared `QualityGovernanceResult` content (via `assessment`, `summary`, `findings`) and the serializer's JSON/Markdown output — never a stored copy of `manifest.json`'s governance fields for cross-run comparison. CAP-080D.1 replaces the one test that did diff `manifest.qualityGovernanceDecision` across two runs with an equivalent check at the runtime-contract boundary (`test_determinism_quality_governance_decision_and_findings`, already present) plus a metadata-only manifest-key check, so a future Markdown formatting change can never fail a runtime regression test, and only a `QualityGovernanceResult` content change can.

---

## Final Architecture Certification (CAP-080D.1)

CAP-080D.1 is the terminal hardening pass over the entire CAP-080 arc — no engine, model, policy, or signature changed; only the one manifest field identified in §D31 was removed, and containment tests now guard the boundary permanently. Certification, restated against the live, wired, hardened system:

1. **Is `QualityGovernanceResult` now the sole runtime contract?** Yes — unchanged since D3/D30; §D31's audit confirms every governance concept resolves to exactly one field on it (recommendations being the one documented gap, not a duplication).
2. **Is the manifest now purely package metadata?** Yes, for Quality Governance — the three surviving keys are a flag and two filenames; `qualityGovernanceDecision` is removed and `tests/unit/test_manifest_purity_boundary.py` enforces the whitelist going forward. (The pre-existing `cp1Verdict` key, frozen at CAP-068, predates this subsystem and is out of this milestone's scope — noted, not touched.)
3. **Is the Execution Package projection-only?** Yes — the serializer computes nothing (D30, unchanged) and the new containment tests confirm `ManifestBuilder`/`ExecutionData`/`ExecutionWriter` import no governance runtime implementation, only the projection-only `serialization` submodule.
4. **Is runtime ownership unique for every governance concept?** Yes, with the one documented, intentional exception: recommendations are not yet propagated into `QualityGovernanceResult` at all (a gap, reserved since D27, not a CAP-080D.1 concern).
5. **Is serialization completely deterministic?** Yes — unchanged; `QualityGovernanceSerializer` is a pure function of `QualityGovernanceResult` (D30).
6. **Is the dependency direction permanently one-way?** Yes — restated and containment-tested in §D31; manifest → runtime and Execution Package → runtime-implementation are both structurally forbidden.
7. **Does the golden dataset compare runtime contracts rather than formatting?** Yes — the one test that diffed a manifest field across runs was replaced with the runtime-contract-level equivalent (already present) plus a metadata-only manifest check (§D31).
8. **Is the manifest protected against future runtime-state growth?** Yes — `tests/unit/test_manifest_purity_boundary.py` whitelists the exact three governance keys and fails if a runtime-state key (decision, score, findings, recommendations) is ever added.
9. **Is the complete CAP-080 architecture now permanently frozen?** Yes — every layer (Rule Evaluation, Assessment, Decision, Governance, Execution Package, manifest) is frozen; CAP-080D.1 closed the one remaining seam without redesigning anything.
10. **Is the repository ready to begin CAP-081 without further Quality Governance work?** Yes.

---

## Final Quality Governance Architecture Certification (CAP-080A.3)

*(Historical — written when the Decision layer was frozen and the subsystem was still unwired from runtime. Superseded by the CAP-080D.1 certification above; kept verbatim as the record of that milestone's own certification.)*
