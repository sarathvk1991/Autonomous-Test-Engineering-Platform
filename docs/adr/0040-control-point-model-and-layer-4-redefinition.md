...
**Amendment note (additive, 2026-07-29, ADR-0044).** The CP3 row above names "SonarQube on generated Java, using the `customqa:*` quality profile (ADR-0037)" without specifying how that assignment gates. ADR-0044 (Layer 3 Automation Engineering Architecture Freeze) specifies it in full, as this table's own binding decision, not a new one: **CP3 does not pass unless a running SonarQube server scans the generated Java and its quality gate — evaluated against the `customqa:*` profile — passes** (a hard gate, not an advisory report). ADR-0044 additionally records two things this table does not: SonarQube's `customqa:*` rules are fed into Layer 3's own generation prompts as constraints, not only evaluated after the fact ("constrain at generation, verify at the gate, so the gate rarely fires"); and the SonarQube adapter itself is unit-tested against a stub/fake (no live server required), with the live gate exercised only by a separate integration test where Sonar actually exists — the same live-vs-stub seam this platform's LLM providers already use. ADR-0044 also reconciles an apparent tension with ADR-0037's own text (which calls the `customqa:*` profile "a Layer 4 asset"): that phrasing describes the profile's storage and versioning, still true and unchanged; *evaluation* against it is this table's own CP3/Layer 3 assignment, which ADR-0044 confirms and specifies. This table's own CP3 row is unchanged by this note — see ADR-0044 D5 for the full specification.
...
**Amendment note (additive, 2026-08-06, ADR-0046).** ADR-0046 (Layer 4 Quality Governance Architecture Freeze) specifies this Decision's suite-level integration governance list in full, as CP5: orphaned-glue detection (deterministic pattern-vs-literal-text matching, primary; semantic similarity, advisory-only secondary), the cross-suite near-duplicate sweep (step definitions only, reusing the reuse engine's own embedding boundary), promotion-wrapping (a suite-level gate running after ADR-0045's per-asset promotion stages and before commit), and aggregate-release cohesion (compiles, per this Decision's own live-tool-not-live-SUT boundary; ambiguous-glue detection via exact pattern collision). This Decision's own list is unchanged — ADR-0046 specifies it, the same relationship ADR-0044 already has with Decision 1's CP3 assignment. ADR-0046 additionally renumbers the deck's own mislabeled "CP5" (whole-suite Sonar governance) and "CP6" (execution-readiness) content to CP7 and CP8 respectively, since this Decision's own CP5 and CP6 (Layer 5's execution control point, per ADR-0044) are both already claimed. See ADR-0046 for the full decision.
