...
- **Continued by:** ADR-0028 (Learning Constitution) — this ADR builds Organizational Memory, the Layer 2 capability that produces Organizational Knowledge; ADR-0028 constitutionally governs Learning, the reserved CAP-086 capability that consumes `OrganizationalMemoryResult` per Recommendation 8 below, ahead of that capability's own architecture freeze. This is a citation-only relationship: no rule in this ADR is modified by ADR-0028's existence.
...
**Policy value tuning — additive correction (2026-08-21).** `OrganizationalMemoryCapabilitySwitches.enable_deterministic_engine` flips `False → True` in the shipped default policy (`OrganizationalMemoryPolicyVersion` 1.0.0 → 1.1.0) now that the engine exists — a versioned policy *value* change, exactly the kind Recommendation 5 anticipated, never a policy *shape* or engine code change, mirroring ADR-0022 §D9's and ADR-0023 §D10's own identical CAP-083B/CAP-084B tuning. `DeterministicOrganizationalMemoryEngine.build` now gates on this switch as the master on/off (mirroring the identical gate in `DeterministicContinuousImprovementEngine.improve` and `DeterministicKnowledgeGraphEngine.build`), before the existing per-stage switches — a step this CAP-085B section should have performed at the time (this ADR's own §D8/Recommendation 5 language already named the trigger: "reserved off until their respective future engine exists," and the engine has existed since CAP-085B), closed retroactively rather than left as a latent inconsistency with the sibling frameworks.
